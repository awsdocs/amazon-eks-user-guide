...
If you're updating to version `1.24` (`containerd` as the default container runtime) or switch to the `containerd` runtime, and already have Fluentd configured for Container Insight, you must migrate Fluentd to Fluent-Bit before updating your cluster to version `1.24`, see [Set up Fluent Bit as a DaemonSet to send logs to CloudWatch Logs](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/Container-Insights-setup-logs-FluentBit.html)\. The switch from the `docker` to the `containerd` container runtime will affect the formatting of the containers' log messages, which some of the configured Fluentd's parsers will fail to parse and generate massive amount of logged error\.  This happens when the Fluentd parsers are configured to only parse log messages in JSON format, hence, failing to parse non-JSON formatted log messages generated by the `containerd` runtime\.
