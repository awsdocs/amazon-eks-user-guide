...
+ The worker node does not include a private DNS entry, resulting in the kubelet log indicating `node "" not found`.  Ensure that the VPC where the worker nodes is created has the DHCP Option Set attribute for **domain_name**.
