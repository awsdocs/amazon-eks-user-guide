...
+ Configure your service accounts to use the global endpoint\. All versions earlier than `1.22` use the global endpoint by default, but version `1.22` and later clusters use the regional endpoint by default\. If your code makes explicit calls to the global endpoint on a `1.22` or later cluster, you'll receive the error if you're using the default endpoint type\. For more information, see [Configuring the AWS Security Token Service endpoint for a service account](configure-sts-endpoint.md)\.