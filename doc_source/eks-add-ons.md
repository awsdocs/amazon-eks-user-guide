...
+ The `eks:addon-cluster-admin` ClusterRoleBinding on every cluster is used by Amazon EKS to manage the lifecycle of add\-ons\. All clusters starting with the following platform versions use the new `ClusterRoleBinding`\.    
