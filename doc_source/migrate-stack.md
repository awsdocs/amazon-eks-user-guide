...
This topic describes how you can create a new node group, migrate your existing applications to the new group, and remove the old node group from your cluster\. You can migrate to a new node group using `eksctl` or the AWS Management Console\.

**To migrate your applications to a new node group with `eksctl`**

This procedure requires `eksctl` version `0.88.0` or later\. You can check your version with the following command:
...
1. When the previous command completes, make sure that all of your nodes have reached the `Ready` state with the following command\.
...
**Note**  
If you attached any additional IAM policies to your old node group IAM role, attach those same policies to your new node group IAM role to maintain that functionality on the new group\. If, for example, you added permissions for the Kubernetes [Cluster Autoscaler](https://github.com/kubernetes/autoscaler/tree/master/cluster-autoscaler), this applies to you\.
...
1. Update the security groups for both node groups so that they can communicate with each other\. For more information, see [Amazon EKS security group considerations](sec-group-reqs.md)\.
   ...
   1. Record the security group IDs for both node groups\. This is shown as the **NodeSecurityGroup** value in the AWS CloudFormation stack outputs\. 
      ...
      You can use the following AWS CLI commands to get the security group IDs from the stack names\. In these commands, `oldNodes` is the AWS CloudFormation stack name for your older node stack\. `newNodes` is the name of the stack that you are migrating to\. Replace every `example-value` with your own values\.
   ...
   1. Add inbound rules to each node security group so that they accept traffic from each other\.
...
1. Drain each of the nodes that you want to remove from your cluster with the following command\.
   ...
   If you're upgrading your nodes to a new Kubernetes version, identify and drain all of the nodes of a particular Kubernetes version \(in this example, *1\.19*\) with the following code snippet\.
...
1. After your old nodes finished draining, revoke the security group inbound rules that you authorized earlier\. Then, delete the AWS CloudFormation stack to terminate the instances\.
**Note**  
If you attached any additional IAM policies to your old node group IAM role, such as adding permissions for the Kubernetes [Cluster Autoscaler](https://github.com/kubernetes/autoscaler/tree/master/cluster-autoscaler)\), you can't delete your AWS CloudFormation stack\. To delete it, you must detach these additional policies from the role\.

   1. Revoke the inbound rules that you created for your node security groups earlier\. In these commands, `oldNodes` is the AWS CloudFormation stack name for your older node stack\. `newNodes` is the name of the stack that you're migrating to\.
...
1. \(Optional\) If you're using the Kubernetes [Cluster Autoscaler](https://github.com/kubernetes/autoscaler/tree/master/cluster-autoscaler), scale the deployment back to one replica\.
**Note**  
Make sure to tag your new Auto Scaling group appropriately \(for example, `k8s.io/cluster-autoscaler/enabled,k8s.io/cluster-autoscaler/my-cluster`\)\. Make sure also to update the command for your Cluster Autoscaler deployment to point to the newly tagged Auto Scaling group\. For more information, see [Cluster Autoscaler on AWS](https://github.com/kubernetes/autoscaler/tree/cluster-autoscaler-release-1.3/cluster-autoscaler/cloudprovider/aws)\.
