...
This procedure requires `eksctl` version `0.73.0` or later\. You can check your version with the following command:
...
1. Launch a new node group with `eksctl` with the following command\. In the comman, replace the *`<example values>`* \(including *`<>`*\) with your own values\. The version number can't be later than the Kubernetes version for your control plane\. Also, it can't be more than two minor versions earlier than the Kubernetes version for your control plane\. We recommend that you use the same version as your control plane\. If you plan to assign IAM roles to all of your Kubernetes service accounts so that pods only have the minimum permissions that they need, and no pods in the cluster require access to the Amazon EC2 instance metadata service \(IMDS\) for other reasons, such as retrieving the current Region, then we recommend blocking pod access to IMDS\. For more information, see [IAM roles for service accounts](iam-roles-for-service-accounts.md) and [Restricting access to the IMDS and Amazon EC2 instance profile credentials](best-practices-security.md#restrict-ec2-credential-access)\. ITo block pod access to IMDS, add the `--disable-pod-imds` option to the following command\.
**Note**  
...
1. Delete the original node group with the following command\. In the command, replace the `<example values>` \(including `<>`\) with your cluster and node group names:
...
**Note**  
You must also tag your new Auto Scaling group appropriately \(for example, `k8s.io/cluster-autoscaler/enabled,k8s.io/cluster-autoscaler/<YOUR CLUSTER NAME>`\) and update the command for your Cluster Autoscaler deployment to point to the newly tagged Auto Scaling group\. For more information, see [Cluster Autoscaler on AWS](https://github.com/kubernetes/autoscaler/tree/cluster-autoscaler-release-1.3/cluster-autoscaler/cloudprovider/aws)\.
