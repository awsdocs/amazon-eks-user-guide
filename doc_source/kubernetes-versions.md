...
Kubernetes supports compatibility between API server components and the kubelets on worker nodes for up to two minor versions - so 1\.14 workers will continue to operate when orchestrated by a 1\.16 control plane\. For more information, see [Kubernetes version and version skew support policy](https://kubernetes.io/docs/setup/version-skew-policy/) in the Kubernetes documentation\.
