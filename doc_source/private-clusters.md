...
+ [AWS Loadbalancer Controller](https://docs.aws.amazon.com/eks/latest/userguide/aws-load-balancer-controller.html) is supported, However while installing you should disable default true configurations such as enable-shield, enable-waf, enable-wafv2 using [Flags](https://kubernetes-sigs.github.io/aws-load-balancer-controller/v2.2/deploy/configurations/#controller-command-line-flags). In addition, [autodiscovery of certificate](https://kubernetes-sigs.github.io/aws-load-balancer-controller/v2.1/guide/ingress/cert_discovery/#discover-via-ingress-rule-host) in the ingress objects based on hostname is not supported as the controller needs to reach ACM which doesnt have vpc endpoint.
