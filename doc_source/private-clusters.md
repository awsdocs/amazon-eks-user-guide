# Deploy private clusters with limited internet access<a name="private-clusters"></a>
...
+ If your Pods use Amazon EFS volumes, then before deploying the [Use Amazon EFS storage](efs-csi.md), the driver's [kustomization\.yaml](https://github.com/kubernetes-sigs/aws-efs-csi-driver/blob/master/deploy/kubernetes/overlays/stable/kustomization.yaml) file must be changed to set the container images to use the same AWS Region as the Amazon EKS cluster\.
