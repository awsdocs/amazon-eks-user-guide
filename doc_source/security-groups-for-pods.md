...
+ You can only use security groups for pods with pods running on [AWS Fargate](fargate.md) if your cluster is 1\.18 or later\. 
...
The security group must allow outbound communication to the cluster security group \(for CoreDNS\) over TCP and UDP port 53\. The cluster security group must also allow inbound TCP and UDP port 53 communication from at least one security group associated to pods\.
