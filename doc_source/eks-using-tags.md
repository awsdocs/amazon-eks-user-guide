...
The AWS generated cost allocation tag, specifically `aws:eks:cluster-name`, is automatically added to Amazon EC2 instances that participate in an Amazon EKS cluster\. Even if the resource isn't launched through Amazon EKS managed node groups, this tag is added\. This tag doesn’t count towards the 50 tags limit that you can associate with a resource\. To break down Amazon EC2 costs by individual Amazon EKS clusters, the account owner must activate this tag in the AWS Billing console or by using the API\. When an AWS Organizations management account owner activates this tag, the tag is also activated for all organization member accounts\.
