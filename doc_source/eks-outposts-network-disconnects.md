...
+ EBS Snapshots cannot be created or deleted if the AWS Outposts cannot access the relevant AWS In-Region APIs (i.e EBS, S3)\.
+ When integrating ALB (Ingress) with AWS Certificate Manager (ACM), certificates are pushed and stored in memory (Outposts ALB Compute instance)\.
Current TLS termination will continue to operate in the event of a disconnect from region\.
Mutating operations in this context will fail (i.e. new ingress definitions and/or new ACM based certificates API operations, ALB compute scale, certificate rotation)\. More info, ACM documentation (https://docs.aws.amazon.com/acm/latest/userguide/troubleshooting-renewal.html).
