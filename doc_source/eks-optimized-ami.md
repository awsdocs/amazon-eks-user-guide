...
For a list of the latest components included in the accelerated AMI, see the `amazon\-eks\-ami`https://github\.com/awslabs/amazon\-eks\-ami/releases\[Releases\] on GitHub\.
...
+ For more information about using Neuron, see [Containers \- Kubernetes \- Getting Started](https://awsdocs-neuron.readthedocs-hosted.com/en/latest/containers/kubernetes-getting-started.html) in the * AWS Neuron Documentation*\.
  ...
  1. Create a file named `nvidia-smi.yaml` with the following contents\. Replace ` tag ` with your desired tag for [nvidia/cuda](https://hub.docker.com/r/nvidia/cuda/tags)\. This manifest launches an [NVIDIA CUDA](https://developer.nvidia.com/cuda-zone) container that runs `nvidia-smi` on a node\.
