...
**Prerequisite**  
Version `0.110.0` or later of the `eksctl` command line tool installed on your device or AWS CloudShell\. To install or update `eksctl`, see [Installing or updating `eksctl`](eksctl.md)\.
...
Create an Amazon EKS `IPv4` cluster with the Amazon EKS latest Kubernetes version in your default AWS Region\. Before running command, make the following replacements:
   ...
   + Replace ** with any [Amazon EKS supported version](kubernetes-versions.md)\. 
   ...
   ```
   eksctl create cluster --name my-cluster --region region-code --version  --vpc-private-subnets subnet-ExampleID1,subnet-ExampleID2 --without-nodegroup
   ```
