...
1. Create a new self\-managed node group for each `ENIConfig` that you configured\.
   ...
   1. Follow the steps in the **Self\-managed nodes** tab of [Launching self\-managed Amazon Linux 2 Linux nodes](launch-workers.md) to create each new self\-managed node group\. After you've opened the AWS CloudFormation template, enter values as described in the instructions\. For the **BootstrapArguments** field, enter the following value\.
