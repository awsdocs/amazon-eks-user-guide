...
1. Create the add\-on using the AWS CLI\. If you want to use the AWS Management Console or `eksctl` to create the add\-on, see [Creating an add\-on](managing-add-ons.md#creating-an-add-on) and specify `coredns` for the add\-on name\. Copy the command that follows to your device\. Make the following modifications to the command, as needed, and then run the modified command\.
   ...
   + Replace *v1\.9\.3\-eksbuild\.2* with the latest version listed in the [latest version table](#coredns-versions) for your cluster version\.
...
**Important**  
This procedure will be removed from this guide on July 1, 2023\. We recommend adding the Amazon EKS type of the add\-on to your cluster instead of self\-managing the add\-on\. To add the Amazon EKS add\-on to your cluster, see [Creating the Amazon EKS add\-on](#coredns-add-on-create)\.
