# Applications<a name="eks-applications"></a>
...
+ Create an external \(for internet\-accessible pods\) or an internal \(for private pods\) [load balancer](load-balancing.md) to balance the traffic load across pods\. The load balancer routes traffic at Layer 4 of the OSI model\.
+ Create an [ALB Ingress Controller on Amazon EKS](alb-ingress.md) to balance the traffic load across pods\. The application load balancer routes traffic at Layer 7 of the OSI model\.
