...
The latest version that we recommend  is version 1\.7\.1\. You can view the different releases available for the plugin, and read the release notes for each version [on GitHub](https://github.com/aws/amazon-vpc-cni-k8s/releases)\.
...
+ Use the following command to print your cluster's CNI version:
  ...
  In this example output, the CNI version is 1\.6\.3, which is earlier than the current recommended version, 1\.7\.1\. Use the following procedure to upgrade the CNI\.
