...
eksctl  
\* \.To remove the Amazon EBS CSI add\-on using `eksctl` Replace ` my-cluster ` with the name of your cluster, and then run the following command\.  
...
1. In the **Remove: aws\-ebs\-csi\-driver** confirmation dialog box, do the following:
   ...
   1. Enter ` `aws-ebs-csi-driver.` Select **Remove**\.

 AWS CLI  
\* \.To remove the Amazon EBS CSI add\-on using the AWS CLI Replace ` my-cluster ` with the name of your cluster, and then run the following command\.  
