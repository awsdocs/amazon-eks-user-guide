...
An Amazon EKS cluster, nodes, and Kubernetes resources are deployed to a VPC\. If you want to use an existing VPC with Amazon EKS, that VPC must meet the requirements that are described in [View Amazon EKS networking requirements for VPC and subnets](network-reqs.md)\. This topic describes how to create a VPC that meets Amazon EKS requirements using an Amazon EKS provided AWS CloudFormation template\. Once you've deployed a template, you can view the resources created by the template to know exactly what resources it created, and the configuration of those resources\.
