...
+ The Amazon EKS platform version number is incremented \(`eks.[replaceable]`n\+1````\)\.
...
Clusters are always created with the latest available Amazon EKS platform version \(`eks.[replaceable]`n` `) for the specified [noloc] minor version that you updated to.` 
