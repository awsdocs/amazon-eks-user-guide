...
+ An existing Amazon EKS cluster\. To deploy one, see \. If you have subnets in the AWS Region where you have AWS Outposts, AWS Wavelength, or AWS Local Zones enabled, those subnets must not have been passed in when you created your cluster\.
...
If you don’t provide a key pair here, the AWS CloudFormation stack creation fails\.
  ...
  1. Record the **NodeInstanceRole** for the node group that was created\. You need this when you configure your Amazon EKS nodes\. NOTE: If you launched nodes inside a private VPC without outbound internet access, make sure to enable nodes to join your cluster from within the VPC\.
