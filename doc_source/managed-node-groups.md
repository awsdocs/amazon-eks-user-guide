...
+ Amazon EKS automatically drains nodes using the Kubernetes API during terminations or updates.
  + Terminating node when AZRebalance or reducing desired count of node, doesn't respect pod disruption budgets. It tries to evict pods on node, but if it takes more than 15 minutes, node will be terminated regardless of whether all pods on node are terminated. If you want to extend the period until node will be terminated, add lifecycle hook to Auto Scaling group. For more information, see [Add lifecycle hooks](https://docs.aws.amazon.com/autoscaling/ec2/userguide/adding-lifecycle-hooks.html).\.
  + Updating managed node group respect the pod disruption budgets that you set for your pods. For more information, see [Managed node update behavior](https://docs.aws.amazon.com/en_us/eks/latest/userguide/managed-node-update-behavior.html)\.
