...
**Note**  
If you are using an Amazon EKS optimized AMI that's `v20220406` or newer, you can use a brand new instance type without upgrading to the latest AMI\. For these AMIs, they will auto\-calculate the necessary `max-pod` values if they aren't listed in the `eni-max-pods.txt` file\.
