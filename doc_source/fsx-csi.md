...
1. Set a few variables to use in the remaining steps\. Replace ` my-csi-fsx-cluster ` with the name of the test cluster you want to createand ` region-code ` with the AWS Region that you want to create your test cluster in\.
...
1. Create a test cluster\.
   ...
   ```
   [✓]  EKS cluster "`my-csi-fsx-cluster`" in "" region is ready
   ```
...
1. Create a Kubernetes service account for the driver and attach the `AmazonFSxFullAccess` AWS\-managed policy to the service account with the following command\. If your cluster is in the AWS GovCloud \(US\-East\) or AWS GovCloud \(US\-West\) AWS Regions, then replace `arn:aws:` with `arn:aws-us-gov:`\.
   ...
   Note the name of the AWS CloudFormation stack that was deployed\. In the previous example output, the stack is named `eksctl-[replaceable]`my\-csi\-fsx\-cluster`-addon-iamserviceaccount-kube-system-[replaceable]`fsx\-csi\-controller\-sa````\.
...
1. Note the ARN for the role that was created\. If you didn’t note it earlier and don’t have it available anymore in the AWS CLI output, you can do the following to see it in the AWS Management Console\.
   ...
   1. Select the stack named `eksctl-[replaceable]`my\-csi\-fsx\-cluster`-addon-iamserviceaccount-kube-system-[replaceable]`fsx\-csi\-controller\-sa````\.
...
1. Verify that the file system is mounted correctly by the application\.
   ...
   tmpfs`3.8G``0``3.8G``0%`/sys/fs/cgroup`192.0.2.0`@tcp:/`abcdef01``1.1T``7.8M``1.1T``1%`/data
