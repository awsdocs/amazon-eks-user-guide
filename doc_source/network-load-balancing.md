...
**Important**  
Do not edit the annotations after creating your service\. If you need to modify annotations, delete the service object and create it again with the desired value for the annotations\.
...
1. Create a service with an internal NLB that load balances to IP targets\. 
   ...
   1. <a name="network-load-balancing-service-sample-manifest"></a>Save the following contents to a file named `sample-service.yaml` file on your computer\. If you're deploying to Fargate nodes, remove the `service.beta.kubernetes.io/aws-load-balancer-scheme: internet-facing` line\. If you're load balancing to pods in a cluster that uses the [IPv6 family](cni-ipv6.md), then remove the *`#`* at the start of the `# service.beta.kubernetes.io/aws-load-balancer-ip-address-type: dualstack` line\.
...
1. <a name="nlb-sample-app-verify-deployment"></a>Verify that the service was deployed\.
   ...
   ```
**Note**  
The value for *10\.100\.240\.137* is different in your ouput and is an IPv6 address if you used the `service.beta.kubernetes.io/aws-load-balancer-ip-address-type: dualstack` annotation in your service spec\. The value for *xxxxxxxxxx*\-*xxxxxxxxxxxxxxxx* is a unique value for your load balancer and the value for *region\-code* is the Region that your cluster is deployed in\. 
...
1. When you're finished with the sample app, remove the deployment, service, and namespace, with the following command\.
