...
The CNI identifies that this packet goes to an external network (not a local pod). If the CNI passes the packet to the node's network stack with the original source IP preserved.
