...
+
.. Set environment variables for your cluster name and account ID:
+
...
[source,bash,subs="verbatim,attributes"]
----
export NODE_ROLE_NAME="my-node-role"
...
* `<your-node-role>` — The IAM role name for the Auto Mode nodes (the same role used in the cluster's compute config).
* `<your-cluster-name>` — Your EKS cluster name.
* `<subnet-id-1>`, `<subnet-id-2>` — Subnet IDs in different Availability Zones where gateway nodes will be provisioned.
