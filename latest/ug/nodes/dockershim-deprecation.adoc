...
For the setting's activation on Amazon EKS AMIs for Amazon Linux 2 in the `containerd` runtime, see `https://github.com/awslabs/amazon-eks-ami/blob/main/templates/al2/provisioners/install-worker.sh[install-worker.sh]` on [.noloc]`GitHub`.
...
📝 https://github.com/search?q=repo:awsdocs/amazon-eks-user-guide+&#91;&#91;dockershim-deprecation,&type=code[Edit this page on GitHub]