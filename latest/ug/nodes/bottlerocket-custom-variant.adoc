...
[abstract]
--
Learn how to create and publish a Bottlerocket AMI with a specific NVIDIA driver version for GPU workloads on Amazon EKS.
--

When you run GPU workloads on Amazon EKS, you choose a Bottlerocket variant that matches your Kubernetes version and accelerator type. Bottlerocket ships validated variants for common configurations, but your organization might need something different: a newer NVIDIA driver branch, a pinned driver version for compliance, additional packages for monitoring, or a hardened baseline that your security team requires.

Because Bottlerocket is fully open-source, you can create a custom variant that meets these needs. This topic shows how to clone an existing variant, swap the NVIDIA driver from the R580 branch to R595 (version 595.71.05), build the image, and register it as a private AMI. A https://github.com/tbuatois/bottlerocket[ready-to-build fork] with `aws-k8s-1.{33,34,35,36}-nvidia-595` variants is also available if you want to skip the creation steps and go straight to building.

*Just need the R595 variant?*
...
* An EC2 instance (or equivalent Linux x86_64 host) with at least 8 cores, 16 GiB RAM, and 150 GB disk
...
The NVIDIA kmod packages are shipped inside `bottlerocket-kernel-kit`. To see what's available, check the https://github.com/bottlerocket-os/bottlerocket-kernel-kit[kernel-kit packages directory] or the https://github.com/bottlerocket-os/bottlerocket-kernel-kit/releases[release notes].
...
NVIDIA driver sources aren't redistributable. The build requires an explicit license acknowledgement:
...
To discover available packages, browse the https://github.com/bottlerocket-os/bottlerocket-kernel-kit[kernel-kit] and https://github.com/bottlerocket-os/bottlerocket-core-kit[core-kit] package directories.
