...
[abstract]
--
This topic explains how to build a custom Bottlerocket variant by swapping the NVIDIA driver branch, and how to register the result as a private AMI for use with Amazon EKS GPU workloads.
--

When you run GPU workloads on Amazon Elastic Kubernetes Service (Amazon EKS), you choose a Bottlerocket variant that matches your Kubernetes version and accelerator type. Bottlerocket ships validated variants for common configurations, but your organization might need something different: a newer NVIDIA driver branch, a pinned driver version for compliance, additional packages for monitoring, or a hardened baseline that your security team requires.

Because Bottlerocket is fully open-source, you can create a custom variant that meets these needs. This topic shows how to clone an existing variant, swap the NVIDIA driver from the R580 branch to R595 (version 595.71.05), build the image, and register it as a private AMI.

[NOTE]
====
A https://github.com/tbuatois/bottlerocket[ready-to-build fork] on the GitHub website with `aws-k8s-1.{33,34,35,36}-nvidia-595` variants is available if you want to skip the creation steps and go straight to building. This fork is community-maintained and is not an official {aws} resource.
====

=== R595 variant quick start

Clone the fork and jump directly to building:
...
Follow Step 1 through Step 4 to set up your build host, then skip to <<bottlerocket-custom-variant-step10>>. Steps 5–9 are already done in the fork.
...
. *Output.* The final `.img.lz4` file is written to `build/images/`. The build is deterministic: the same `Twoliter.toml` pins and variant `Cargo.toml` produce the same image regardless of host.
...
Note the following about the repository structure:
...
[NOTE]
====
When you complete this walkthrough, terminate the EC2 instance and deregister any AMIs you no longer need to avoid ongoing charges. For instructions, see <<bottlerocket-custom-variant-cleanup>>.
====
...
For a reproducible build, check out a tagged release (for example, `git checkout v1.62.1`). For the latest packages, stay on `develop`.
...
== Going further: other combinations you can build
...
For information about available packages, see the https://github.com/bottlerocket-os/bottlerocket-kernel-kit[kernel-kit] on the GitHub website and the https://github.com/bottlerocket-os/bottlerocket-core-kit[core-kit] on GitHub.

[IMPORTANT]
====
Bottlerocket's root filesystem is immutable. You can't install packages at runtime. Every package in the kits is cross-compiled specifically for Bottlerocket — standard upstream RPMs don't work. If you need software that isn't already in a kit, consider https://github.com/bottlerocket-os/bottlerocket#bootstrap-containers-settings[bootstrap containers] or https://github.com/bottlerocket-os/bottlerocket#host-containers[host containers] as a runtime alternative.
====
