...
Because Bottlerocket is fully open-source, you can create a custom variant that meets these needs. This topic shows how to build a variant that uses the NVIDIA R595 driver (version 595.71.05). You then build the image and register it as a private AMI.

[#bottlerocket-custom-variant-quick-start]
== (Optional) Use a pre-built community fork

If you want to skip steps 5–9, you can use a community-maintained fork that already contains the R595 variant definitions. This fork is not an official {aws} resource, and we don't support it.
...
. *Fetch dependencies.* Twoliter (the Bottlerocket build orchestrator) reads `Twoliter.toml` and pulls three OCI artifacts from `public.ecr.aws/bottlerocket`:
+
