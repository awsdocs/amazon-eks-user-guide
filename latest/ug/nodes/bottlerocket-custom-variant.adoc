...
[abstract]
--
This topic explains how to build a custom Bottlerocket variant by swapping the NVIDIA driver branch. It also shows how to register the result as a private Amazon Machine Image (AMI) for Amazon Elastic Kubernetes Service (Amazon EKS) graphics processing unit (GPU) workloads.
--
...
[NOTE]
====
The open-source community maintains forks of the Bottlerocket repository that include pre-built variant definitions for the R595 driver. We don't endorse or support these forks. If you use one, verify that it meets your security and compliance requirements before deploying.
====

If you use a community fork that already contains the R595 variant definitions, follow <<bottlerocket-custom-variant-step1,Step 1>> through <<bottlerocket-custom-variant-step4,Step 4>> to set up your build host, then skip to <<bottlerocket-custom-variant-step10,Step 10>>. Steps 5–9 are already done in those forks.
