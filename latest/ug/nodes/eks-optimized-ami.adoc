...
* For Kubernetes version `1.23`, you can use an optional bootstrap flag to test migration from Docker to `containerd`. For more information, see <<containerd-bootstrap>>.
...
* Amazon EKS will no longer publish Amazon Linux 2 (AL2) and AL2-optimized AMIs after November 26th, 2025. After this date, these AMIs will no longer receive updates, including minor releases, patches, bug fixes, or security fixes. Additionally, Kubernetes version `1.32` is the last version for which Amazon EKS will release AL2 AMIs. From version `1.33` onwards, Amazon EKS will continue to release AL2023 and Bottlerocket based AMIs.
