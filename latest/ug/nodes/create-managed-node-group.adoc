...
*** *Capacity type* – Select a capacity type. For more information about choosing a capacity type, see <<managed-node-group-capacity-types>>. You can't mix different capacity types within the same node group. If you want to use both capacity types, create separate node groups, each with their own capacity and instance types. See <<ml-node-groups>> for information on provisioning and scaling GPU-accelerated worker nodes.
