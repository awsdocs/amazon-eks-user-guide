...
The EKS-optimized Amazon Linux 2023 AMIs automatically execute nodeadm init in two phases through separate systemd services: nodeadm-config runs before user data execution, while nodeadm-run executes afterward. The nodeadm-config service establishes baseline configurations for containerd and kubelet before user data runs. The nodeadm-run service then performs two main steps: first, it sets up system aspects using a modular approach where each component (such as local disk configuration and networking) is configured independently, and second, it starts both containerd and kubelet while applying any configuration changes specified in the NodeConfig object within the instance's UserData. If the nodeadm init command is ran an additional time, via user data or a custom AMI, it will cause issues with ENI ownership, leading to incorrect routes being created for secondary ENIs on the node.
