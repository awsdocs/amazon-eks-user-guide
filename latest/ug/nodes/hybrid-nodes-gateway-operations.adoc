...
[#hybrid-nodes-gateway-instance-recommendations]
...
|8 GiB
|Up to 30 Gbps
|Network-optimized; recommended for production
...
|16 GiB
|Up to 12.5 Gbps
|Suitable if co-locating other workloads on gateway nodes
|===
...
|21 GiB
|Up to 25 Gbps
|Previous-generation network-optimized, cost-effective

|`c6in.4xlarge`
|16
|32 GiB
|Up to 50 Gbps
|Maximum throughput for very heavy workloads

|`c5n.4xlarge`
|16
|42 GiB
|Up to 25 Gbps
|High vCPU count for extreme packet rates
|===

Monitor network utilization using the gateway metrics (see <<hybrid-nodes-gateway-metrics>>) and adjust the instance type as needed.
...
The VXLAN interface is created at startup on every gateway pod, not just the leader. This ensures the standby is ready to forward traffic within 3–5 seconds of acquiring the lease during failover.
