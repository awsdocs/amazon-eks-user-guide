...
----
+
If you're upgrading your nodes to a new [.noloc]`Kubernetes` version, identify and drain all of the nodes of a particular [.noloc]`Kubernetes` version (in this case, [.replaceable]`{k8s-n-2}`) with the following code snippet.
+
[source,bash,subs="verbatim,attributes"]
----
K8S_VERSION={k8s-n-2}
...
📝 https://github.com/search?q=repo%3Aawsdocs%2Famazon-eks-user-guide+%5B%23migrate-stack%5D&type=code[Edit this page on GitHub]