...
[IMPORTANT]
====
* When specifying an AMI, Amazon EKS does not validate the Kubernetes version embedded in your AMI against your cluster's control plane version. You are responsible for ensuring that your custom AMI conforms to the https://kubernetes.io/releases/version-skew-policy[Kubernetes version skew policy]:
+
** `kubelet` must not be newer than `kube-apiserver`
** The `kubelet` version on your nodes must be equal to or up to 3 minor versions behind your control plane version (for Kubernetes 1.28+)
+
