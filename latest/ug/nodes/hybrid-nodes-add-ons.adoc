...
In addition to the Amazon EKS add-ons in the tables above, the <<prometheus,Amazon Managed Service for Prometheus Collector>>, and the <<aws-load-balancer-controller,{aws} Load Balancer Controller>> for <<alb-ingress,application ingress>> (HTTP) and <<network-load-balancing,load balancing>> (TCP/UDP) are compatible with hybrid nodes. The https://github.com/cert-manager/aws-privateca-issuer[{aws} Private Certificate Authority cert-manager plugin] is also compatible with EKS Hybrid Nodes. 
...
[NOTE]
====
The EKS Pod Identity Agent add-on is not compatible with hybrid nodes running Bottlerocket.
====
