...
* If your cluster uses a subnet with <<vpc-interface-endpoints,PrivateLink>>, make sure that you have configured the Amazon ECR FIPS PrivateLink endpoints. Amazon ECR supports FIPS endpoints through PrivateLink. For more information, see link:AmazonECR/latest/userguide/vpc-endpoints.html[Amazon ECR interface VPC endpoints,type="documentation"].
