...
----
+
You can replace [.replaceable]`{k8s-n}` with a eks/latest/userguide/platform-versions.html[platform-version,type="documentation"] that's the same. Or, it should be up to one version earlier than the Kubernetes version running on your control plane. We recommend that you keep your nodes at the same version as your control plane. You can also replace [.replaceable]`amazon-linux-2` with a different AMI type. For more information, see <<retrieve-ami-id>>.
+
