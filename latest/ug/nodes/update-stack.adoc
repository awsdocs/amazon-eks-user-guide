...
----
. (Optional) Verify that you're using the latest version of the https://github.com/aws/amazon-vpc-cni-k8s[Amazon VPC CNI plugin for Kubernetes]. You might need to update your [.noloc]`Amazon VPC CNI plugin for` [.noloc]`Kubernetes` version to use the latest supported instance types. For more information, see <<managing-vpc-cni>>.
