...
{aws} supports AWS Application Load Balancer (ALB) and Cilium for Kubernetes Ingress for workloads running on EKS Hybrid Nodes. The decision to use ALB or Cilium for Ingress is based on the source of application traffic. If application traffic originates from an AWS Region, we recommend using AWS ALB and the AWS Load Balancer Controller. If application traffic originates from the local on-premises or edge environment, we recommend using Cilium's built-in Ingress capabilities, which can be used with or without load balancer infrastructure in your environment.

image::images/EKS-Hybrid-Nodes-ingress.png[EKS Hybrid Nodes Ingress,scaledwidth=50%,align="center"]
...
- Cilium BGP Control Plane enabled following the instructions in <<hybrid-nodes-cni-bgp-control-plane>>. If you do not want to use BGP, you must use an alternative method to make your on-premises pod CIDRs routable on your on-premises network. If you do not make your on-premises pod CIDRs routable, ALB will not be able to register or contact your pod IP targets.
- See <<alb-ingress>>, <<lbc-helm>>, and link:eks/latest/best-practices/load-balacing.html[Best Practices for Load Balancing,type="documentation"] for additional information about AWS Application Load Balancer and AWS Load Balancer Controller.
...
- eksctl installed in your command-line environment, see the <<eksctl,Setup eksctl instructions>> for more information.
...
----
+
. Replace the values for cluster name, AWS Region, and AWS account ID with your settings and run the following command.
+
[source,bash,subs="verbatim,attributes,quotes"]
----
eksctl create iamserviceaccount \
    --cluster=[.replaceable]`CLUSTER_NAME` \
    --namespace=kube-system \
    --name=aws-load-balancer-controller \
    --attach-policy-arn=arn:aws:iam::[.replaceable]`AWS_ACCOUNT_ID`:policy/AWSLoadBalancerControllerIAMPolicy \
    --override-existing-serviceaccounts \
    --region [.replaceable]`AWS_REGION` \
    --approve
----
+
...
----
+
. Install the AWS Load Balancer Controller. Replace the values for cluster name, AWS Region, VPC ID, and AWS Load Balancer Controller Helm chart version with your settings. You can find the latest version of the Helm chart by running `helm search repo eks/aws-load-balancer-controller --versions`. If you are running a mixed mode cluster with both hybrid nodes and nodes in AWS Cloud, you can run the AWS Load Balancer Controller on cloud nodes following the instructions at <<hybrid-nodes-mixed-lbc>>.
+
[source,bash,subs="verbatim,attributes,quotes"]
----
...
----
+
. Provisioning the ALB for your Ingress resource may take a few minutes. Once the ALB is provisioned, your Ingress resource will have an address assigned to it that corresponds to the DNS name of the ALB deployment. The address will have the format `<ingress-name>-<random-string>.<region>.elb.amazonaws.com`.
+
[source,bash]
----
...
The table below summarizes the differences between Cilium Ingress and Cilium Gateway as of *Cilium version 1.17.x*.
...
- Cilium must be configured with `nodePort.enabled` set to `true` and with `envoy.enabled` set to `true` as shown in the examples below. If you are using Cilium's kube-proxy replacement feature, you do not need to set `nodePort.enabled` to `true`.
- Cilium Gateway can be deployed in load balancer (default) or host network mode. When Cilium is installed in host network mode, the Service of type LoadBalancer mode is disabled. Host network mode is useful for environments that do not have load balancer infrastructure, see <<hybrid-nodes-ingress-cilium-host-network>> for more information.
...
----
+
. Create a file called `cilium-gateway-values.yaml` with the following contents.
+
[source,yaml]
----
gatewayAPI:
...
Cilium Gateway is enabled on Gateway objects by setting the `gatewayClassName` to `cilium`. The Service that Cilium creates for Gateway resources can be configured with fields on the Gateway object. Common annotations used by Ingress and Gateway controllers to configure the load balancer infrastructure can be configured on the Gateway object's `infrastructure` field. When using Cilium's LoadBalancer IPAM, the IP address to use for the Service of type LoadBalancer can be configured on the Gateway object's `addresses` field. For more information on Gateway configuration, see the link:https://gateway-api.sigs.k8s.io/reference/spec/#gateway[Kubernetes Gateway API specification].
...
Cilium and the Kubernetes Gateway specification support the GatewayClass, Gateway, HTTPRoute, GRPCRoute, and ReferenceGrant resources. See link:https://gateway-api.sigs.k8s.io/api-types/httproute/HTTPRoute[HTTPRoute] and link:https://gateway-api.sigs.k8s.io/api-types/grpcroute/GRPCRoute[GRPCRoute] specifications for the list of available fields. See the examples in the <<hybrid-nodes-ingress-cilium-gateway-deploy>> section of this page and the examples in the link:https://docs.cilium.io/en/stable/network/servicemesh/gateway-api/gateway-api/#examples[Cilium documentation] for how to use and configure these resources.
...
----
+
. Create a file named `my-gateway.yaml` with the following contents.
+
[source,yaml]
----
---
...
----
+
. Confirm the Gateway resource and corresponding Service were created. At this stage, it is expected that the `ADDRESS` field of the Gateway resource is not populated with an IP address or hostname, and that the Service of type LoadBalancer for the Gateway resource similarly does not have an IP address or hostname assigned. This is because the `service.beta.kubernetes.io/aws-load-balancer-type: "external"` annotation was set on the Gateway resource, and there is no AWS Load Balancer Controller running in the environment to provision the load balancer infrastructure.
+
[source,bash]
----
...
----
+
. Proceed to <<hybrid-nodes-ingress-cilium-loadbalancer>> to configure the Gateway resource to use an IP address allocated by Cilium LB IPAM, and <<hybrid-nodes-ingress-cilium-nodeport>> or <<hybrid-nodes-ingress-cilium-host-network>> to configure the Gateway resource to use NodePort or host network addresses.
...
- Cilium must be configured with `nodePort.enabled` set to `true` and with `envoy.enabled` set to `true` as shown in the examples below. If you are using Cilium's kube-proxy replacement feature, you do not need to set `nodePort.enabled` to `true`.
- With `ingressController.loadbalancerMode` set to `dedicated`, Cilium creates dedicated Services for each Ingress resource. With `ingressController.loadbalancerMode` set to `shared`, Cilium creates a shared Service of type Load Balancer for all Ingress resources in the cluster. When using the `shared` load balancer mode, the settings for the shared Service such as `labels`, `annotations`, `type`, and `loadBalancerIP` are configured in the `ingressController.service` section of the Helm values. See the link:https://github.com/cilium/cilium/blob/v1.17.5/install/kubernetes/cilium/values.yaml#L887[Cilium Helm values reference] for more information.
...
- Set `ingressController.service.annotations` to `service.beta.kubernetes.io/aws-load-balancer-type: "external"` in the Helm values to prevent the legacy AWS cloud provider from creating a Classic Load Balancer for the default `cilium-ingress` Service created by the link:https://github.com/cilium/cilium/blob/main/install/kubernetes/cilium/templates/cilium-ingress-service.yaml[Cilium Helm chart].

=== Prerequisites

. Helm CLI installed in yourcommand-line environment, see the <<helm,Setup Helm instructions>> for more information.
. Cilium installed following the instructions in <<hybrid-nodes-cni>>.

=== Procedure

. Create a file called `cilium-ingress-values.yaml` with the following contents.
...
----
+
. Create a file named `my-ingress.yaml` with the following contents.
+
[source,yaml]
----
apiVersion: networking.k8s.io/v1
...
----
+
. Confirm the Ingress resource and corresponding Service were created. At this stage, it is expected that the `ADDRESS` field of the Ingress resource is not populated with an IP address or hostname, and that the shared or dedicated Service of type LoadBalancer for the Ingress resource similarly does not have an IP address or hostname assigned. This is because the `service.beta.kubernetes.io/aws-load-balancer-type: "external"` annotation was set on the Ingress resource, and there is no AWS Load Balancer Controller running in the environment to provision the load balancer infrastructure.
+
[source,bash]
----
kubectl get ingress my-ingress
...
----
+
. Proceed to <<hybrid-nodes-ingress-cilium-loadbalancer>> to configure the Ingress resource to use an IP address allocated by Cilium LB IPAM, and <<hybrid-nodes-ingress-cilium-nodeport>> or <<hybrid-nodes-ingress-cilium-host-network>> to configure the Ingress resource to use NodePort or host network addresses.
...
kubectl patch gateway -n default my-gateway --type=merge -p '{
  "spec": {
    "addresses": [{"type": "IPAddress", "value": [.replaceable]"`LB_IP_ADDRESS`"}]
  }
}'
----
+
...
kubectl patch ingress my-ingress --type=merge -p '{
    "metadata": {"annotations": {"lbipam.cilium.io/ips": [.replaceable]"`LB_IP_ADDRESS`"}}
}'
----
+
. Create a file named `cilium-lb-ip-pool.yaml` with a `CiliumLoadBalancerIPPool` resource to configure the Load Balancer IP address range for your Ingress / Gateway resources. 
...
----
+
. Create the `CiliumBGPAdvertisement` resource to advertise the LoadBalancer IP address for the Ingress / Gateway resources. If you are not using Cilium BGP, you can skip this step. The LoadBalancer IP address used for your Ingress / Gateway resource must be routable on your on-premises network for you to be able to query the service in the next step.
+
[source,yaml]
----
apiVersion: cilium.io/v2alpha1
...
----
+
. Access the Service of type NodePort using the IP addresses of your nodes and the NodePort captured above.
+
[source,bash]
----
...
----
+
. Access the Service using the IP addresses of your nodes and the listener port for the `cilium-gateway-my-gateway` Service (`8111`).
+
[source,bash]
----
...
----
+
. Access the Service using the IP addresses of your nodes and the `sharedListenerPort` for the `cilium-ingress` Service (`8111`).
+
[source,bash]
----
curl -s http://10.80.146.32:8111/details/1 | jq
