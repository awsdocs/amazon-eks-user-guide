...
[source,bash,subs="verbatim,attributes"]
----
curl -O https://s3.us-west-2.amazonaws.com/amazon-eks/cloudformation/2025-11-xx/amazon-eks-nodegroup.yaml
----
...
[NOTE]
====
The following two steps are only needed if using the Configmap  Authentication Mode within the EKS Cluster. Additionally, f you launched nodes inside a private VPC without outbound internet access, make sure to enable nodes to join your cluster from within the VPC.
====
