...
* Running Amazon EKS Hybrid Nodes on cloud infrastructure, including {aws} Regions, {aws} Local Zones, {aws} Outposts, or in other clouds, is not supported.
* A single Amazon EKS cluster can be used to run hybrid nodes and nodes in {aws} Regions, {aws} Local Zones, or {aws} Outposts. See <<hybrid-nodes-webhooks#hybrid-nodes-considerations-mixed-mode, Considerations for mixed mode clusters>> for more information.
