...
* Amazon EKS Hybrid Nodes supports the same Kubernetes versions and deprecation schedule as Amazon EKS, including standard and extended Kubernetes version support. For more information on Kubernetes versions in Amazon EKS, see <<kubernetes-versions>>.
* Amazon EKS Hybrid Nodes can be used with new or existing Amazon EKS clusters.
