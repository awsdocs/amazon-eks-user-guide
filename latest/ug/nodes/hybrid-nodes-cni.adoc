...
- {aws} recommends using Cilium's built-in BGP functionality to make your pod CIDRs routable on your on-premises network. For more information on how to configure Cilium BGP with hybrid nodes, see <<hybrid-nodes-bgp>>.
...
----
helm repo add cilium https://public.ecr.aws/cilium/cilium
----
+
...
- The example below disables the Envoy Layer 7 (L7) proxy that Cilium uses for L7 network policies and ingress. For more information, see <<hybrid-nodes-network-policy>> and <<hybrid-nodes-ingress-cilium>>. 
