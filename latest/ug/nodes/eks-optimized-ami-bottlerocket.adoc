...
* Bottlerocket supports Amazon EC2 instances with GPUs. For more information, see <<ml-eks-optimized-ami>>.
...
* To build a custom Bottlerocket variant with a specific NVIDIA driver, see <<bottlerocket-custom-variant>>.
...
include::bottlerocket-custom-variant.adoc[leveloffset=+1]
