...
If you need to rollback your cluster control plane to a previous Kubernetes version, you must first rollback your managed node groups to ensure compliance with the Kubernetes version skew policy. Worker nodes cannot run a version newer than the control plane.

You can rollback a managed node group using the `UpdateNodegroupVersion` API with the previous Kubernetes version:

[source,bash,subs="verbatim,attributes,quotes"]
----
aws eks update-nodegroup-version \
  --cluster-name [.replaceable]`my-cluster` \
  --nodegroup-name [.replaceable]`my-nodegroup` \
  --kubernetes-version [.replaceable]`1.30` \
  --region [.replaceable]`us-west-2`
----
