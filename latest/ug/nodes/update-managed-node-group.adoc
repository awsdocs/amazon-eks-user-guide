...
You can upgrade a node group to the same version as the control plane's [.noloc]`Kubernetes` version. For example, if you have a cluster running [.noloc]`Kubernetes` `{k8s-n}`, you can upgrade nodes currently running [.noloc]`Kubernetes` `{k8s-n-1}` to version `{k8s-n}` with the following command.
...
📝 https://github.com/search?q=repo:awsdocs/amazon-eks-user-guide+&#91;&#91;update-managed-node-group,&type=code[Edit this page on GitHub]