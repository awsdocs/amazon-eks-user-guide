...
Added `wafv2:ListWebACLs` permission in `AmazonEKSLoadBalancingPolicy` to allow Amazon EKS Auto Mode Controller to list {aws} WAF Web ACLs when reconciling Application Load Balancer associations.
Added `gateway.eks.amazonaws.com.alb/stack`, `gateway.eks.amazonaws.com.alb/resource`, `gateway.eks.amazonaws.com.nlb/stack`, and `gateway.eks.amazonaws.com.nlb/resource` tag keys to the resource-tagging allowlist in `AmazonEKSLoadBalancingPolicy`. This allows Amazon EKS Auto Mode Controller to manage {aws} resources for Kubernetes Gateway API (ALB and NLB) workloads.

[.update,date="2026-08-24"]
=== Deterministic Argo CD capability endpoint
[.update-ulink]
link:eks/latest/userguide/working-with-argocd.html#argocd-endpoint-url[type="documentation"]

Documented the format of the EKS Capability for Argo CD endpoint. The endpoint hostname is deterministic, so you can construct it from your capability name, cluster name, {aws} account ID, and {aws} Region, or retrieve it from the `serverUrl` field of `describe-capability`.

[.update,date="2026-07-28"]
=== {aws} PrivateLink for the cluster OIDC endpoint
[.update-ulink]
link:eks/latest/userguide/vpc-interface-endpoints.html#oidc-vpc-interface-endpoints[type="documentation"]

Amazon EKS now supports {aws} PrivateLink for the cluster OIDC discovery/JWKS endpoint (`com.amazonaws.region-code.oidc-eks`), letting you reach the endpoint used by IAM roles for service accounts (IRSA) privately from your VPC without internet egress.

[.update,date="2026-07-21"]
=== EFA support for static capacity in EKS Auto Mode
[.update-ulink]
link:eks/latest/userguide/create-node-class.html#static-network-interfaces[type="documentation"]
...
Added `AmazonEKSPodIdentityPolicy` to the list of access policies in `AmazonEKSServiceRolePolicy` that the service-linked role can associate and dissociate from cluster access entries. With this addition, Amazon EKS can manage pod identity credentials for workloads running on EKS clusters.

[.update,date="2026-06-18"]
=== Control plane egress routing
[.update-ulink]
link:eks/latest/userguide/control-plane-egress.html[type="documentation"]

Added support for customer-routed control plane egress (`controlPlaneEgressMode=CUSTOMER_ROUTED`), letting you manage the network path from the cluster control plane to resources in your VPC.
...
Added `elasticloadbalancing:SetRulePriorities` permission in `AmazonEKSLoadBalancingPolicy` to allow Amazon EKS Auto Mode Controller to set priorities of the specified listener rules for ALB.
Added `ec2:DescribeIpamPools` permission in `AmazonEKSLoadBalancingPolicy` to describe ipam pool

[.update,date="2026-06-02"]
=== Kubernetes version `1.36`
[.update-ulink]
link:eks/latest/userguide/kubernetes-versions-standard.html#kubernetes-1-36[type="documentation"]

Added Kubernetes version `1.36` to Amazon EKS.
...
Added permissions to `AmazonEKSComputePolicy` to allow Amazon EKS Auto Mode to launch instances from Interruptible Capacity Reservations in your account.
...
Added a new {aws} managed policy: `AmazonEKSBlockStoragePolicyV2`
...
Added new API action `ec2:DescribeInstanceTypes` to `AmazonEBSCSIDriverPolicy` to enable the EBS CSI Driver to dynamically retrieve volume limit/card information at runtime.
...
Added `elasticloadbalancing:DescribeLoadBalancers` permission in `AmazonEKSLoadBalancingPolicy` to allow Amazon EKS Auto Mode Controller to describe LoadBalancer 

[.update,date="2026-04-23"]
=== Kubernetes version `1.32` for local clusters on {aws} Outposts
[.update-ulink]
link:eks/latest/userguide/eks-outposts-platform-versions.html[type="documentation"]

Added Kubernetes version `1.32` support for local Amazon EKS clusters on {aws} Outposts.
...
Added `elasticloadbalancing:DescribeCapacityReservation` permission in `AmazonEKSLoadBalancingPolicy` to allow Amazon EKS Auto Mode Controller to describe LoadBalancer capacity units
...
Added `ec2:DescribeInstanceStatus` and `iam:ListInstanceProfiles` permissions in `AmazonEKSServiceRolePolicy`. These permissions allow Amazon EKS Auto Mode to discover EC2 health events for managed instances and cleanup managed instance profiles
...
Added permissions to `AmazonEKSComputePolicy` to allow Amazon EKS Auto Mode to launch instances into the EC2 Placement Groups in your account.
...
Introduced `AmazonEBSCSIDriverPolicyV2` and `AmazonEBSCSIDriverEKSClusterScopedPolicy` as more restrictive alternatives to `AmazonEBSCSIDriverPolicy`. `AmazonEBSCSIDriverPolicyV2` scopes the EBS CSI driver to volumes and snapshots tagged with `ebs.csi.aws.com/cluster` set to `true`. `AmazonEBSCSIDriverEKSClusterScopedPolicy` scopes the driver to volumes and snapshots belonging to a specific EKS cluster using the `ebs.csi.aws.com/cluster-name` tag.
...
Added permissions to `AmazonEKSServiceRolePolicy` to allow Amazon EKS Auto Mode to launch instances into the EC2 Placement Groups in your account.
...
Updated `shield:CreateProtection` and `shield:DeleteProtection` permission in `AmazonEKSLoadBalancingPolicy`. This allows Amazon EKS Auto Mode Controller to add shield protection.

[.update,date="2026-04-07"]
=== Warm pool support for managed node groups
[.update-ulink]
link:eks/latest/userguide/warm-pools-managed-node-groups.html[type="documentation"]

Amazon EKS managed node groups now support Amazon EC2 Auto Scaling warm pools. Warm pools maintain a pool of pre-initialized EC2 instances that are ready to quickly join your cluster when scaling out, decreasing latency for applications with long boot times.
...
Added `elasticloadbalancing:ModifyIpPools` and updated `elasticloadbalancing:RegisterTargets` permission in `AmazonEKSLoadBalancingPolicy`. This allows Amazon EKS Auto Mode Controller to support setting IPAM pool for ALB and support multi-cluster and custom targetgroupbindings respectively
...
Added `ec2:ModifyNetworkInterfaceAttribute` permission in `AmazonEKSNetworkingPolicy`. This allows Amazon EKS Auto Mode Controller to modify network interface attributes related to EC2 instances
...
Added `autoscaling:PutWarmPool`, `autoscaling:DeleteWarmPool`, and `autoscaling:DescribeWarmPool` permissions to `AWSServiceRoleForAmazonEKSNodegroup` policy. This allows Amazon EKS Managed Nodegroups to manage underlying ASG warm pool resources throughout the node group lifecycle.
...
Removed the "eks" prefix requirement in the name of the target instance profile for the `iam:GetInstanceProfile` permission in `AmazonEKSServiceRolePolicy`. This allows Amazon EKS Auto Mode to validate and utilize custom instance profiles in NodeClasses without requiring the "eks" naming prefix.

[.update,date="2026-01-27"]
=== Kubernetes version `1.35`
[.update-ulink]
link:eks/latest/userguide/kubernetes-versions.html#kubernetes-1-35[type="documentation"]
 
Added Kubernetes version `1.35` support for new clusters and version upgrades.
...
Added `ec2:LockSnapshot` permission to `AmazonEBSCSIDriverPolicy` to allow the EBS CSI Driver to lock EBS Snapshots directly. 
...
Added permissions to `AmazonEKSClusterPolicy` to allow Amazon EKS to clean up elastic network interfaces created by the VPC CNI. This is required so that EKS can clean up elastic network interfaces that are left behind if the VPC CNI quits unexpectedly.
...
Amazon EKS now supports adding node taints to managed node groups.
...
With this update, you can get audit and diagnostic logs directly from the Amazon EKS control plane. You can use these CloudWatch logs in your account as reference for securing and running clusters.
...
[.update-ulink]
link:https://github.com/kubernetes-sigs/aws-alb-ingress-controller[Amazon EKS ALB ingress controller GitHub]
