...
* The `kubectl` command line tool is installed on your device or {aws} CloudShell. The version can be the same as or up to one minor version earlier or later than the [.noloc]`Kubernetes` version of your cluster. To install or upgrade `kubectl`, see <<install-kubectl>>.
...
. Replace [.replaceable]`{k8s-n}` with any xref:kubernetes-versions[Amazon EKS supported version,linkend=kubernetes-versions].
