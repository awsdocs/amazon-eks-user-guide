...
* You can't use <<security-groups-for-pods,Assign security groups to individual pods>> with [.noloc]`Pods` running on [.noloc]`Windows` nodes.
* You can't use <<cni-custom-network,custom networking>> with [.noloc]`Windows` nodes.
