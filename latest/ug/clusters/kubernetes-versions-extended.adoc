...
* `kube-proxy` versions `1.21` and lower are not compatible with Kubernetes `1.25` due to the deprecation of (`discovery.k8s.io/v1beta1`) of `EndpointSlice`. Upgrade `kube-proxy` to `1.25`, or kube-proxy will not function, causing other pods which rely on kube-proxy to fail. 
