...
. Select the control plane scaling tier that you would like to provision for the cluster from various scaling tier options such as XL, 2XL, 4XL, and 8XL.
. Select other cluster configuration options as needed. On the final step select *Create cluster*. Note it may take several minutes for cluster creation to complete.
