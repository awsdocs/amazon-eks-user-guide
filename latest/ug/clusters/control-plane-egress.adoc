...
= Configure control plane egress routing
...
By default, Amazon EKS manages the egress networking from the Kubernetes control plane to resources in your VPC. With _control plane egress routing_, you can change this behavior so that you manage the network path yourself. This gives you full control over how traffic from the control plane elastic network interfaces (ENIs) reaches your VPC resources. You can route through your own NAT gateways, firewalls, or inspection appliances.

// TODO(diagram): add an architecture diagram here showing: kube-apiserver -> cross-account ENI in customer VPC subnet -> route table -> egress device (e.g. NAT gateway) -> internet -> webhook/OIDC endpoint. Source PNG to be created and committed to images/ per the team diagram workflow (WorkDocs source).
image::images/control-plane-egress-overview.png[Control plane egress routing overview]
...
. Choose *Add cluster*, then *Create*.
...
For {aws} CloudFormation, set `ControlPlaneEgressMode: CUSTOMER_ROUTED` in `ResourcesVpcConfig`. For Terraform, set `control_plane_egress_mode = "CUSTOMER_ROUTED"` in the `vpc_config` block.
...
// TODO(PMT): confirm and document the minimum Kubernetes version, the AWS Regions where CUSTOMER_ROUTED is available at launch
...
For example, the following SCP denies cluster creation unless the caller specifies `CUSTOMER_ROUTED`:
...
Use this policy to enforce that all new clusters in your organization use the `CUSTOMER_ROUTED` egress mode.
