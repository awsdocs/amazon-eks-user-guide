...
[NOTE]
====
This topic covers data plane egress (nodes and workloads without outbound internet access). To control how egress traffic from the cluster _control plane_ reaches your VPC, see <<control-plane-egress>>.
====
...
|Amazon EKS cluster OIDC endpoint (required to set up IAM roles for service accounts from inside the VPC)
|com.amazonaws.[.replaceable]`region-code`.oidc-eks
