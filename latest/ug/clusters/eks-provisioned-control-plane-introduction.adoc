...
[cols=",",options="header",]
|===
|Provisioned Control Plane Scaling Tier |Tier capacity
...
plane’s tier utilization. The
`apiserver++_++flowcontrol++_++current++_++executing++_++seats++_++total`
metric shows the number of concurrent request seats being processed by
the API server, which you can compare against your tier’s API server
request concurrency limit. The
`scheduler++_++schedule++_++attempts++_++total` metric indicates the
rate at which the default kube-scheduler schedules pods on nodes. The
`etcd++_++mvcc++_++db++_++total++_++size++_++in++_++use++_++in++_++bytes`
metric allows you to monitor your cluster state storage utilization. All
these metrics are published as CloudWatch vended metrics and are
accessible through the Amazon EKS console. Additionally, these metrics
...
* *Exiting limitations* – You cannot exit an EKS Provisioned Control
...
To get started, see https://quip-amazon.com/mN9PAHJk1CYb[++[++WIP++]++
