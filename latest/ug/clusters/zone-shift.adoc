...
* If you're using <<automode,EKS Auto Mode>>, EKS Auto Mode automatically stops provisioning new nodes in the impaired AZ. EKS Auto Mode also suspends voluntary disruption actions such as consolidation and drift that would affect the impaired AZ, which stops voluntary instance termination in the impaired AZ to prevent retry storms. Pods with strict scheduling requirements that target the impaired AZ (such as persistent volume bindings, node affinity, or strict topology spread constraints) do not result in new node launch attempts to that AZ.
...
Self-managed Karpenter support is available with ARC zonal shift and zonal autoshift in EKS with https://github.com/aws/karpenter-provider-aws[Karpenter version 1.12 or greater].
