...
Amazon EKS uses the `AWSServiceRoleForAmazonEKS` Service Linked Role (SLR) to access information about your cluster. Amazon EKS may automatically create a read-only access entry, so that cluster insights can read information about your Kubernetes cluster. 
