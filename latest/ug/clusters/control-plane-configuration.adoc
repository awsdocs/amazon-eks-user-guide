...
The controller manager runs the terminated pod garbage collector (the pod GC controller), which deletes terminated pods (pods in the `Succeeded` or `Failed` phase) once the number of terminated pods in the cluster exceeds a threshold. The `terminatedPodGcThreshold` parameter sets that threshold.
...
| `podGcControllerConfig.terminatedPodGcThreshold`
| The number of terminated pods that can exist before the terminated pod garbage collector starts deleting terminated pods.
| `10000` to `12500`
| `12500`
|===
...
To configure this parameter, your cluster must be on Amazon EKS Provisioned Control Plane. Lowering the threshold increases the garbage collection work the controller performs against the cluster database (etcd), because each pass becomes eligible to query, process, and delete more terminated pods. Provisioned Control Plane clusters pre-allocate control plane capacity that is always ready for demanding workloads, so they are sized to absorb that additional load. For more information, see <<eks-provisioned-control-plane>>.

==== Considerations for the terminated pod garbage collection threshold

* *Lowering the threshold deletes excess terminated pods immediately.* If you lower the threshold (for example, from `12500` to `10000`), the garbage collection controller starts force-deleting the oldest terminated pods on its next cycle until the count reaches the new threshold. The reduction is not gradual.
