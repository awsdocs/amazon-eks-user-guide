...
To learn more about these components and how they work, see <<eks-architecture>>.
...
It's important to note that in an Amazon EKS environment, `etcd` storage is limited to 8 GiB per https://etcd.io/docs/v3.5/dev-guide/limit/#storage-size-limit[etcd upstream] guidance. You can optionally monitor a metric for the current database size via ` kubectl get --raw=/metrics | grep "apiserver_storage_size_bytes"`. Note, that if your cluster has a Kubernetes version below `1.28`, be sure to replace [.replaceable]`apiserver_storage_size_bytes` with `apiserver_storage_db_total_size_in_bytes`.
