...
** You must have [.noloc]`Windows` support enabled for your cluster. For more information, see <<windows-support>>.
...
[IMPORTANT]
====
Even if your subnet has available IP addresses, if the subnet does not have any contiguous `/28` blocks available, you will see the following error in the [.noloc]`Amazon VPC CNI plugin for Kubernetes` logs.
...
📝 https://github.com/search?q=repo%3Aawsdocs%2Famazon-eks-user-guide+%5B%23cni-increase-ip-addresses-procedure%5D&type=code[Edit this page on GitHub]