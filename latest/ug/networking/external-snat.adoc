...
* Your Pods can communicate with internet resources if the node that they're running on either has a link:AWSEC2/latest/UserGuide/using-instance-addressing.html#concepts-public-addresses[public,type="documentation"] or link:vpc/latest/userguide/vpc-eips.html[elastic,type="documentation"] IP address and is in a link:vpc/latest/userguide/configure-subnets.html#subnet-basics[public subnet,type="documentation"] whose link:vpc/latest/userguide/VPC_Route_Tables.html[route table,type="documentation"] has a route to an link:vpc/latest/userguide/VPC_Internet_Gateway.html[internet gateway,type="documentation"], or is in a link:vpc/latest/userguide/configure-subnets.html#subnet-basics[private subnet,type="documentation"] whose route table has a route to a link:vpc/latest/userguide/vpc-nat-gateway.html[NAT gateway,type="documentation"]. We recommend deploying nodes to private subnets, whenever possible.
...
* You want the Pod's source IP address to be preserved when traffic leaves the node, with SNAT performed by a link:vpc/latest/userguide/vpc-nat-gateway.html[NAT gateway,type="documentation"] instead of the node — for example, so network appliances or logs observe the Pod's IP address. Your nodes must be in a link:vpc/latest/userguide/configure-subnets.html#subnet-basics[private subnet,type="documentation"] with a route to a NAT gateway, because Pod-sourced traffic can't route through an internet gateway.
