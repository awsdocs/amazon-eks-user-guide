...
** Replace [.replaceable]`v1.20.0-eksbuild.1` with the latest version listed in the latest version table for your cluster version.
...
Version v1.13.2 was a pivotal release that https://github.com/aws/amazon-vpc-cni-k8s/pull/2402[introduced iptables-wrapper], which automatically detects the appropriate iptables backend (iptables-legacy or iptables-nft) for inserting chains and rules. This change aligned with the upstream Kubernetes decision to move away from the legacy backend due to performance limitations.
