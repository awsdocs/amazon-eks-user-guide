...
* If you're installing the controller on the same cluster where the role (`AmazonEKSLoadBalancerControllerRole`) was originally created, go to <<lbc-helm-install,Step 2: Install AWS Load Balancer Controller>> after verifying the role exists.
