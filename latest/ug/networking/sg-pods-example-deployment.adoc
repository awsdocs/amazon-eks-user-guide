...
If you have a Pod with Amazon EC2 instances, you must configure the plugin before you use this procedure. For more information, see <<security-groups-pods-deployment>>.

. Create a Kubernetes namespace to deploy resources to. You can replace [.replaceable]`my-namespace` with the name of a namespace that you want to use.
+
...
+
.. Copy the following contents to your device. Replace the example values with your own and then run the modified command. If you replace [.replaceable]`my-role`, make sure that it's the same as the value you specified for the selector in a previous step.
+
[source,yaml,subs="verbatim,attributes"]
----
