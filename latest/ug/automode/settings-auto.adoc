...
a|
*Multi cluster target groups*

- Understand how the load balancer controller manages target group membership
- Enable MultiCluster support to share a target group across multiple Kubernetes clusters
- Support arbitrary targets from other sources in a shared target group
|<<auto-multi-cluster-target-groups>>

a|
*Extending RBAC for managed controllers*

- Understand how EKS Auto Mode managed controllers authenticate to your cluster
- Grant additional Kubernetes RBAC to the managed controllers via the `eks:managed` group
- Scope the grant with `Role` / `RoleBinding` and `resourceNames` for least privilege
|<<auto-managed-rbac>>
...
- Configure kernel lockdown security mode
|<<auto-advanced-security>>

a|
*Cost optimization*

- Understand how consolidation, bin-packing, and right-sizing reduce compute costs
- Identify configurations that block consolidation (`do-not-disrupt`, restrictive PDBs)
- Set NodePool `limits` as a cost ceiling
- Restrict instance families and sizes to control all-in cost
- Apply recommended patterns for bursty CI/CD workloads
|<<auto-cost-control,Cost optimization in EKS Auto Mode>>
...
include::auto-multi-cluster-target-groups.adoc[leveloffset=+1]

include::auto-multi-cluster-target-groups-example.adoc[leveloffset=+1]

include::auto-managed-rbac.adoc[leveloffset=+1]

include::auto-managed-rbac-example.adoc[leveloffset=+1]
...
include::auto-advanced-security.adoc[leveloffset=+1]

include::auto-cost-control.adoc[leveloffset=+1]
