...
Unlike dynamic node pools that scale based on pod scheduling demands, static-capacity node pools maintain a predetermined number of nodes that you specify. This provides predictable costs and consistent resource availability for applications that need guaranteed compute capacity.
