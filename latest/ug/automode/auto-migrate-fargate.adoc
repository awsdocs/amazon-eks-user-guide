...
eks-auto-mode     default     0       True    3d14h
...
kubectl apply -f deployment_fargate.yaml
----
. Check that deployment is running in the EKS Automode:
+
[source,bash,subs="verbatim,attributes"]
----
...
----
. Verify there is no Fargate node running and that there is no deployment running in the EKS Automode manage nodes:
+
[source,bash,subs="verbatim,attributes"]
----
kubectl get node -owide
