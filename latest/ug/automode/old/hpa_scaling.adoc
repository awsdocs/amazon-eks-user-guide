...
Notice that the pods are intentional about how much CPU and memory they request:
...
Now that the application is running, and they're being intentional about how many resources they need, it's time to configure an HPA rule to scale-out when the CPU usage goes above 70%. To do so, run this command:
