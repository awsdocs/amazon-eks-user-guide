...
In `Auto` mode, EKS Auto Mode allocates a `/28` prefix (16 IPs) to each node up front and adds another `/28` when pod demand exceeds the current block's capacity, so 16 is a per-node minimum rather than a cap. The following examples compare IP allocation between the two modes.
