...
* *Supported sysctls* – Only the sysctls exposed as fields on `advancedCompute.kernel.sysctl` are supported. Each field enforces a documented minimum and maximum, and NodeClass admission rejects values outside that range.
