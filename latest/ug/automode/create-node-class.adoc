...
[IMPORTANT]
====
**Pod-sparse or high-scale workloads: choose your pod IP allocation mode intentionally.**

EKS Auto Mode uses prefix delegation (`/28` prefixes) by default, which reserves 16 pod IPs per node whether the node runs 1 pod or 16. For pod-sparse workloads (a few pods per node, common in ML or GPU workloads, or anti-affinity-heavy workloads) targeting more than a few hundred nodes per Availability Zone, the default can exhaust `/20` pod subnets before the fleet reaches target size. The `AssignPrivateIpAddresses` and `CreateNetworkInterface` failures that follow surface as `Pending` pods that never schedule.
...
spec:
  ...
  securityGroupSelectorTerms:
    - tags:
        Name: "eks-cluster-sg"
  advancedNetworking:
    ipv4PrefixSize: "32"    # secondary IP mode; the only other valid value is "Auto"
----

`advancedNetworking.ipv4PrefixSize` accepts two values: `Auto` (default; prefix delegation; higher pod-creation velocity; reserves 16 IPs per node) and `"32"` (secondary IP mode; 1 IP per pod; adds per-pod `AssignPrivateIpAddresses` calls; pods-per-node capped by the instance type's ENI IP-limit).
