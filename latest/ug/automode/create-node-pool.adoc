...
|spot
|Capacity types include `spot`, `on-demand`, and `reserved`. If you do not specify a capacity type, Karpenter prioritizes `reserved`, then `spot`, then `on-demand`.
...
|eks.amazonaws.com/instance-numa-nodes
|2
|Number of NUMA nodes on the instance. Present only on supported instance types.

|eks.amazonaws.com/instance-min-cpus-per-numa
|16
|Number of CPUs on the smallest NUMA node of the instance. Present only on supported instance types.

|eks.amazonaws.com/instance-min-memory-per-numa
|65536
|Number of mebibytes of memory on the smallest NUMA node of the instance. Present only on supported instance types.
...
|true
|Whether the capacity reservation is interruptible. Only present on nodes with `karpenter.sh/capacity-type: reserved`.
...
. Create an EKS cluster with both `nodePools` and `nodeRoleArn` values empty.
...
For details on each value, see https://karpenter.sh/docs/concepts/disruption/[Disruption] in the Karpenter documentation.
