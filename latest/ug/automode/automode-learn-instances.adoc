...
* If the configured ephemeral storage in the NodeClass is smaller than the NVMe local storage for the instance, EKS Auto Mode eliminates the need for manual configuration and automatically takes the following actions:
** Uses a smaller (20 GiB) EBS data volume to reduce costs.
** Formats and configures the NVMe local storage for ephemeral data use, including setting up a RAID 0 array if there are multiple NVMe drives.
