...
. After the DNS request goes through the DNS filter allow list, the proxy forwards it to CoreDNS,
. CoreDNS in turn sends the request to the External DNS Resolver (Amazon Route 53 Resolver) to get the list of IP addresses behind the domain name.
...
. Retrieve the Service CIDR for your cluster.
+
For IPv4 clusters:
+
[source,bash]
...
----
. Derive the CoreDNS IP address from the Service CIDR:
+
