...
You must have the `aws` and `eksctl` tools installed. You must be logged into the {aws} CLI with sufficient permissions to manage {aws} resources including: EC2 instances, EC2 networking, EKS clusters, and IAM roles. For more information, see <<setting-up>>. 
