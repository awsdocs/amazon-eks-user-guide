...
EKS Auto Mode charges a management fee per vCPU-hour for managed instances. This fee is in addition to the underlying EC2 instance cost. When EKS Auto Mode selects instance types, it optimizes for EC2 price — the management fee is not currently factored into instance selection decisions.

This means that for workloads using Spot instances, EKS Auto Mode might select large instance types where deep Spot discounts make the EC2 price attractive, but the management fee on the larger instance can offset those savings.

To manage your all-in cost:

* *Constrain instance sizes* — Use the `eks.amazonaws.com/instance-cpu` label to limit maximum instance sizes in your NodePool requirements:
+
[source,yaml]
----
requirements:
...
* *Review your largest instances* — Identify nodes with the highest vCPU counts in your cluster and evaluate whether the workloads on those nodes could be distributed across smaller instances.
* *Factor the fee into capacity planning* — When comparing EKS Auto Mode costs against self-managed alternatives, include the management fee in your per-node cost calculations.

For current management fee pricing, see link:eks/latest/pricing/[Amazon EKS pricing,type="documentation"].
...
=== Recommended safe defaults for CI/CD workloads
...
|NodePool `limits`
|Set a CPU/memory ceiling based on your maximum expected concurrency plus 20% buffer for node replacement overlap.

|Instance categories
|Restrict to `c` and `m` families. Exclude accelerated, burstable (`t`), and high-memory instance types.

|Instance sizes
|Limit to 4–32 vCPUs to prevent a single large node from accumulating cost during idle periods.
...
EKS Auto Mode enforces a maximum node lifetime of 21 days. When a node reaches its expiry, EKS Auto Mode replaces it with a new node. During replacement:

* A new node is launched before the old node is terminated.
* For a brief period, both the old and replacement nodes are running simultaneously, resulting in temporarily doubled capacity for affected workloads.
* Disruption budgets control the pace of replacement — restrictive budgets extend the overlap period.

For clusters with large or many nodes, this overlap can create periodic cost increases. To minimize the impact:

* *Reduce maximum node lifetime* — Setting a shorter expiry (for example, 7 days) creates more frequent but smaller replacement events, spreading the cost more evenly.
* *Review disruption budgets* — Ensure your disruption budgets permit timely replacement. Budgets that are too restrictive extend the period during which both old and new nodes are running.
* *Right-size instances* — Smaller instances reduce the absolute cost of the overlap period.
