...
* link:hpc/efa/[Elastic Fabric Adapter (EFA), type="marketing"] — For information on managing EFA devices in your EKS cluster, see <<device-management-efa>>.
...
Instead of setting the `eks.amazonaws.com/instance-gpu-name` you might use `eks.amazonaws.com/instance-family` to specify the instance family. For other well-known labels which influence scheduling review, see <<auto-supported-labels>>.
