...
This tutorial demonstrates how Amazon EKS Auto Mode simplifies launching GPU-based workloads. Amazon EKS Auto Mode streamlines operations beyond the cluster itself by automating key infrastructure components providing compute, networking, load balancing, storage, and Identity Access and Management capabilities out of the box. 

Amazon EKS Auto Mode includes the drivers required for certain instance types, such as NVIDIA and {aws} Neuron drivers. You do not have to install or update these drivers. 

EKS Auto Mode manages the drivers for: 
...
* NVIDIA GPU link:ec2/latest/instancetypes/ac.html[Amazon EC2 accelerated instances,type="documentation"]
** Including the https://github.com/NVIDIA/k8s-device-plugin[NVIDIA device plugin for Kubernetes]. EKS Auto Mode includes this plugin, and is not visible as a daemon set within your cluster. 

Amazon EKS Auto Mode also includes support for link:hpc/efa/[Elastic Fabric Adapter,type="marketing"].

//GDC: What does the EFA thing mean?

Amazon EKS Auto Mode eliminates the toil of GPU driver management. 
...
The following provides an example of how to launch GPU-based workloads with Amazon EKS Auto Mode.
...
In this example, you will create a NodePool for NVIDIA inference based workloads that requires 45GB GPU memory. With EKS Auto Mode, you use Kubernetes scheduling constraints to define your instance requirements. 
...
spec:
  nodeSelector:
    eks.amazonaws.com/instance-gpu-memory: "45776"
    eks.amazonaws.com/compute-type: auto
  restartPolicy: OnFailure
  containers:
  - name: nvidia-smi
    image: nvidia/cuda:12.8.0-cudnn-devel-ubuntu20.04
    args:
    - "nvidia-smi"
    resources:
      requests:
...
Wait about one minute, and check the nodes in your cluster. You should now see a new node provisioned in your Amazon EKS Auto Mode cluster:
...
You can see Amazon EKS Auto Mode launched a `g6e.2xlarge` rather than an `g6.2xlarge` as the workload required an instance with at least 45GB of GPU memory, according to the following Kubernetes scheduling constraints:
...
In addition, GPU workloads might be sensitive to specific GPU memory requirements, you might set a `eks.amazonaws.com/instance-gpu-memory` node selector on workloads so they schedule on nodes that have the required GPU memory resources. For other well-known labels which influence scheduling review, see <<auto-supported-labels>>.
...
The following NodePool has an `eks.amazonaws.com/instance-family` set that says, only launch instances of Inferentia and Trainium family:
