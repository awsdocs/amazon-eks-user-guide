...
For version rollbacks, EKS Auto Mode automatically rolls back worker nodes before the control plane is reverted, honoring your configured disruption controls. For more information, see <<rollback-automode>>.
...
*** Automatically handles link:AWSEC2/latest/UserGuide/spot-instance-termination-notices.html[EC2 Spot Instance interruption notices,type="documentation"], EC2 instance health/scheduled maintenance events, and EC2 instance status check failures
...
include::auto-observability.adoc[leveloffset=+1]
