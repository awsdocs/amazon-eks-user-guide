...
----
include::iam_snippet:cb3ec475-7e7f-44d9-8e3f-54e5f092b59e[]
----
...
[NOTE]
====
You can only deploy workloads depending on EKS Auto Mode StorageClasses on EKS Auto Mode nodes. If you have a cluster with mixed types of nodes, you need to configure your workloads to run only on EKS Auto Mode nodes. For more information, see <<associate-workload>>. 
====
...
[%header,cols="2"]
|===
|Existing VMK annotation
|VAC parameter

| `ebs.csi.eks.amazonaws.com/volumeType` | type
| `ebs.csi.eks.amazonaws.com/iops` | iops
| `ebs.csi.eks.amazonaws.com/throughput` | throughput
|===
+
