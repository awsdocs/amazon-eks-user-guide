...
. Create a new key policy that permits access to the KMS key.
** Use the sample IAM policy below to create the key policy, taking care to insert the correct account ID and region. For more information, see 
link:kms/latest/developerguide/key-policies.html["Key policies in AWS KMS",type="documentation"] in the KMS User Guide. 
. Attach the policy to the KMS key.
** Use the {aws} KMS console to find the key you created. In the *Key Policy* tab, select *Edit*, copy in your policy from the previous step, and save your changes.
...
[[auto-install-snapshot-controller,auto-install-snapshot-controller.title]]
...
📝 https://github.com/search?q=repo:awsdocs/amazon-eks-user-guide+&#91;&#91;create-storage-class,&type=code[Edit this page on GitHub]