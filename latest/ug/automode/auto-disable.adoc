...
If any managed Security Group is not deleted at the end of this process, you can delete it manually using the instructions in link:vpc/latest/userguide/deleting-security-groups.html[Delete a security group,type="documentation"].

== Disable EKS Auto Mode ({aws} CLI)

Use the following command to disable EKS Auto Mode on an existing cluster. 

You need to have the `aws` CLI installed, and be logged in with sufficient permissions to manage EKS clusters. For more information, see <<setting-up>>. 
