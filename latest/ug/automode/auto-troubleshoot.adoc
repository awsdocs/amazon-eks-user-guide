...
For Clusters with Control Plane logs enabled, you can gain insights into Karpenter's actions and decision-making process by querying the logs. This can be particularly useful for troubleshooting Auto Mode issues related to node provisioning, scaling, and termination. To view Karpenter-related events, use the following CloudWatch Logs Insights query:
...
This query filters for specific link: https://github.com/kubernetes-sigs/karpenter/blob/main/pkg/events/reason.go[Karpenter-related events] in the kube-apiserver audit logs. The events include various disruption states, scheduling failures, capacity issues, and node-related problems. By analyzing these logs, you can gain a better understanding of:

. Why Karpenter is taking certain actions
. Any issues preventing proper node provisioning or scaling
...
The results will show you a timeline of Karpenter-related events, helping you troubleshoot issues and understand the behavior of Auto Mode in your cluster. To review Karpenter actions on a specific node, you can add the below filter to the aforementioned query:
...
[NOTE]
====
This query assumes you have enabled Control Plane logging for your EKS cluster. If you haven't enabled it yet, you can do so by following the instructions in <<control-plane-logs>>.
====
