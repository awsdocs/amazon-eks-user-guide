...
* Use a Kubernetes `NodeDiagnostic` resource to capture network traffic on a node. For more steps, see <<auto-get-tcpdump>>.
...
The recommended way of retrieving logs from an EKS Auto Mode node is to use a `NodeDiagnostic` resource. For these steps, see <<auto-get-logs>>.
...
. On the **Account attributes** card, under **Settings**, choose **Managed resource visibility**.
. Choose **Manage**, and then select the visibility toggle for managed instances.
. Choose **Save changes**.
...
For more information, see link:AWSEC2/latest/UserGuide/amazon-ec2-managed-instances.html#managed-resource-visibility-settings["Managed resource visibility settings",type="documentation"] in the _Amazon EC2 User Guide_.
--
...
. Choose the settings (gear) icon in the top right of the Event history table and enable the *Error code* column.
. Review the *Error code* column for permission-related errors such as `AccessDenied`, `UnauthorizedOperation`, and `InvalidClientTokenId`. To narrow the list, filter by *Event source* (for example, `ec2.amazonaws.com` or `eks.amazonaws.com`) and a time range.
...
[NOTE]
====
You are charged for each analysis that is run by the VPC Reachability Analyzer. For pricing details, see link:vpc/pricing/[Amazon VPC Pricing,type="marketing"].
====
...
. Choose "Create and Analyze Path"
. The analysis might take a few minutes to complete. If the analysis results indicate failed reachability, it will indicate where the failure was in the network path so you can resolve the issue.
...
This query filters for specific https://github.com/kubernetes-sigs/karpenter/blob/main/pkg/events/reason.go[Karpenter-related events] in the kube-apiserver audit logs. The events include various disruption states, scheduling failures, capacity issues, and node-related problems. By analyzing these logs, you can gain a better understanding of:
...
. Select "Logs Insights" from the left navigation pane
. Choose the log group for your EKS cluster's control plane logs
...
[NOTE]
====
To use this query, control plane logging must be enabled on your EKS cluster. If you haven't done this yet, please refer to <<control-plane-logs>>.
====
...
* https://repost.aws/articles/ARLpQOknr5Rb-w5iAT9sUBpQ[How to troubleshoot common scaling issues in EKS Auto-Mode?]
* https://repost.aws/articles/ARPcmFS1POTgqPCBdcZFp6BQ[How do I troubleshoot custom nodepool and nodeclass provisioning issues in Amazon EKS Auto Mode?]
* https://repost.aws/en/articles/ARLhrdl45TRASGkvViwtBG0Q[How do I troubleshoot EKS Auto Mode built-in node pools with Unknown Status?]
