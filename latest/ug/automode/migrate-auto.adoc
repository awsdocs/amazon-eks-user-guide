...
. **Modify volume retention policy**: Change the existing PersistentVolume's (PV's) `persistentVolumeReclaimPolicy` to `Retain` to ensure the underlying EBS volume is not deleted.
