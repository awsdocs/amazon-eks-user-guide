...
Clusters upgraded through in-place upgrade can be rolled back to the previous minor version within 7 days of the upgrade completing. To rollback to a version in extended support, you must first change the cluster's upgrade policy to `EXTENDED`. Clusters that were automatically upgraded at the end of extended support cannot be rolled back. For more information, see <<rollback-cluster>>.
