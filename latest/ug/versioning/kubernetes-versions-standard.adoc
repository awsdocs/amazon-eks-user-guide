...
Kubernetes `1.35` is now available in Amazon EKS. For more information about Kubernetes `1.35`, see the https://kubernetes.io/blog/2026/01/27/kubernetes-v1-35-release/[official release announcement].

[IMPORTANT]
====
* Cgroup v1 Support Removed: Kubernetes 1.35 deprecates cgroups v1 support, meaning the kubelet will refuse to start on nodes using cgroups v1 by default.
** AL2023: AL2023 uses cgroups v2 by default and aligns with Kubernetes upstream behavior.
*** Action required: Customers who manually configured AL2023 to use cgroups v1 must either https://kubernetes.io/docs/concepts/architecture/cgroups/[migrate to cgroups v2] or manually set `failCgroupV1: false` in kubelet configuration.
** Bottlerocket: Bottlerocket 1.35 uses cgroups v2 by default, however sets `failCgroupV1: false` in the kubelet configuration, maintaining backward compatibility.
** Fargate: Fargate continues to use cgroups v1.
...
* *Windows Server 2025 Support:* EKS 1.35 adds support for https://docs.aws.amazon.com/eks/latest/userguide/eks-optimized-windows-ami.html[Windows Server 2025] for self-managed node groups.
