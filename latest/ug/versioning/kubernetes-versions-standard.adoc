...
** AL2023 users: AL2023 uses cgroups v2 by default and aligns with Kubernetes upstream behavior.
...
** Bottlerocket users: Bottlerocket 1.35 uses cgroups v2 by default, however sets `failCgroupV1: false` in the kubelet configuration, maintaining backward compatibility.
** Fargate users: Fargate continues to use cgroups v1.
