...
NOTE: If you roll back from a version under standard support to a version under extended support, extended support charges resume for that cluster.
...
* Starting with Amazon EKS version `1.30` or newer, any newly created managed node groups will automatically default to using Amazon Linux 2023 (AL2023) as the node operating system. For more information about specifying the operating system for a managed node group, see <<create-managed-node-group>>.
