...
. *Review controller logs*: If log delivery is configured, query controller logs for errors and reconciliation details
...
If you have configured controller log delivery (see <<capabilities-controller-logs>>), you can query logs to identify reconciliation errors, resource conflicts, and configuration issues.
...
kubectl describe [.replaceable]`resource-kind resource-name` -n [.replaceable]`namespace`

# View resource status conditions
kubectl get [.replaceable]`resource-kind resource-name` -n [.replaceable]`namespace` -o jsonpath='{.status.conditions}'
