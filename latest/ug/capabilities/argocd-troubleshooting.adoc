...
Use the finalizer list that the previous command printed.
Set it to the finalizers you want to keep, leaving out only the blocking one.
Replace [.replaceable]`finalizers-to-keep` with a comma-separated list of quoted finalizer names:

[source,bash,subs="verbatim,attributes,quotes"]
----
kubectl patch [.replaceable]`resource-kind` [.replaceable]`resource-name` -n [.replaceable]`my-namespace` \
  --type merge -p '{"metadata":{"finalizers":[[.replaceable]``finalizers-to-keep``]}}'
----
...
A resource can carry finalizers from more than one controller, and they are not in a guaranteed order. Removing the first entry can delete the wrong finalizer. This leaves the blocking one in place and the resource still stuck.
