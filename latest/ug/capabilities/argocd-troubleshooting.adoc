...
Use the finalizer list that the previous command printed, and set it to the finalizers that you want to keep, leaving out only the blocking one:

[source,bash,subs="verbatim,attributes,quotes"]
----
kubectl patch [.replaceable]`resource-kind` [.replaceable]`resource-name` -n [.replaceable]`my-namespace` \
  --type merge -p '{"metadata":{"finalizers":["finalizer-to-keep"]}}'
----

Replace `finalizer-to-keep` with the finalizers you want to remain, as a comma-separated list of quoted names.
...
A resource can carry finalizers from more than one controller, and they are not in a guaranteed order, so removing the first entry can delete the wrong finalizer—leaving the blocking one in place and the resource still stuck.
