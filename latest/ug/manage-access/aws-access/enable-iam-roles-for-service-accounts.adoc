...
====
+
To reach the cluster OIDC endpoint privately from your VPC, create a VPC interface endpoint for it (`com.amazonaws.<region-code>.oidc-eks`) with private DNS enabled. For more information, see link:eks/latest/userguide/vpc-interface-endpoints.html#oidc-vpc-interface-endpoints[Access the cluster OIDC endpoint using {aws} PrivateLink,type="documentation"].
