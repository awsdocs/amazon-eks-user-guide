...
. (Optional) Select *Disable session tags* to disable the default session tags that Pod Identity automatically adds when it assumes the role.
. (Optional) Toggle *Configure session policy* to configure an IAM policy to apply additional restrictions to this Pod Identity association beyond the permissions defined in the IAM policy attached to the IAM role.
+
NOTE: A session policy can only be applied when the *Disable session tags* setting is checked. 
+
