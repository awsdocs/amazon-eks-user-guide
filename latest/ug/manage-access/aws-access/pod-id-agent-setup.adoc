...
[IMPORTANT]
====
The nodes where the EKS Pod Identity Agent runs must be able to access the EKS Auth API. If you are using private subnets for your nodes, you must set up an {AWS} PrivateLink interface endpoint for the EKS Auth API. For more information, see <<vpc-interface-endpoints>>.
====
