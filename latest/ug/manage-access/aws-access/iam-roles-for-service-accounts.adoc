...
To reach the cluster OIDC endpoint privately from your VPC, create a VPC interface endpoint for it (`com.amazonaws.[.replaceable]``region-code``.oidc-eks`) with private DNS enabled. For more information, see <<vpc-interface-endpoints>>.

Alternatively, you can run the command outside the VPC (for example, in {aws} CloudShell) or create a split-horizon conditional resolver. For an example, see the https://github.com/aws/containers-roadmap/issues/2038[Amazon EKS feature request] on GitHub.
====
