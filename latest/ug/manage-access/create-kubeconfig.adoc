...
* An IAM user or role with permission to use the `eks:DescribeCluster` API action for the cluster that you specify. For more information, see <<security-iam-id-based-policy-examples>>. If you use an identity from your own OpenID Connect provider to access your cluster, then see https://kubernetes.io/docs/reference/access-authn-authz/authentication/#using-kubectl[Using kubectl] in the Kubernetes documentation to create or update your `kubeconfig` file.
...
. Choose the name of the cluster you want to connect to.
. On the cluster details page, choose *Connect* from the top-right action bar.
