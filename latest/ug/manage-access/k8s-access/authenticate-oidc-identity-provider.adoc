...
* You can associate up to 10 OIDC identity providers with a single Amazon EKS cluster running Kubernetes version 1.32 or later. The combined size of all OIDC provider configurations must be less than 12 KB. On clusters running versions earlier than 1.32, you can associate only one OIDC identity provider.
* Associating more than one OIDC identity provider requires an Amazon EKS cluster running Kubernetes version 1.32 or later.
* The issuer URL (`issuerUrl`) must be unique across all OIDC identity providers associated with the cluster. You can't associate the same provider more than once, even if the client ID (audience) is different.
...
On Amazon EKS clusters running Kubernetes version 1.32 or later, you can associate additional OIDC identity providers with your cluster. To associate multiple providers, associate them one at a time. Call the association one time for each provider, and wait for each cluster update to complete before you start the next one.

[NOTE]
====
Associating or disassociating an OIDC identity provider is a cluster update. The cluster enters the `UPDATING` state, and the change can take several minutes to be fully applied to the cluster's API servers. You can track the progress of the update with the link:eks/latest/APIReference/API_DescribeUpdate.html[DescribeUpdate,type="documentation"] operation.
====
