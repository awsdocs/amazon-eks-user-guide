...
[NOTE]
====
If you want to use both Amazon S3 file system and Amazon EFS storage, you must attach both the `AmazonS3FilesCSIDriverPolicy` and the `AmazonEFSCSIDriverPolicy` managed policies to your IAM role. For more information about Amazon EFS storage, see <<efs-csi>>.
====
...
... Choose *Create*.
... Repeat the above steps to create a second Pod Identity association with the same IAM role, but input `efs-csi-node-sa` for the *Kubernetes service account* field.
...
. Save the following contents to a file named `direct-s3-read-policy.json`.
+
[source,json,subs="verbatim,attributes"]
----
{
