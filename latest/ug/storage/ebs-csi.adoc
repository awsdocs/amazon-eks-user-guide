...
The Amazon EBS CSI plugin requires IAM permissions to make calls to {aws} APIs on your behalf. If you don't do these steps, attempting to install the add-on and running `kubectl describe pvc` will show `failed to provision volume with StorageClass` along with a `could not create volume in EC2: UnauthorizedOperation` error. For more information, see https://github.com/kubernetes-sigs/aws-ebs-csi-driver/blob/master/docs/install.md#set-up-driver-permissions[Set up driver permissions] on GitHub.  
...
NOTE: You can create a self-managed policy with further scoped-down permissions. Review link:aws-managed-policy/latest/reference/AmazonEBSCSIDriverPolicyV2.html[`AmazonEBSCSIDriverPolicyV2`,type="documentation"] and create a custom IAM policy with reduced permissions. If migrating from `AmazonEBSCSIDriverPolicy`, see https://github.com/kubernetes-sigs/aws-ebs-csi-driver/issues/2918[EBS CSI Driver policy migration].
...
+
.. In the *Filter policies* box, enter `AmazonEBSCSIDriverPolicyV2`.
.. Select the check box to the left of the `AmazonEBSCSIDriverPolicyV2` returned in the search.
.. Choose *Next*.
