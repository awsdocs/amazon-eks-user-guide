...
----
+
Add a new condition block for the "sub" claim. Replace `<region-code>` with the {aws} Region that your cluster is in. Replace `EXAMPLED539D4633E53DE1B71EXAMPLE` with your cluster's OIDC provider ID.
+
[source,json,subs="verbatim,attributes"]
----
"StringLike": {
...
----
... Keep the `"StringEquals"` operator for the `:aud` condition and use `"StringLike"` for the `:sub` condition.
