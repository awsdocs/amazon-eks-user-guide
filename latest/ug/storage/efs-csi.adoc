...
[.topic]
[#efs-csi]
= Use elastic file system storage with Amazon EFS and Amazon S3 Files
:info_titleabbrev: Amazon EFS and Amazon S3 Files

[abstract]
--
The Amazon EFS Container Storage Interface (CSI) driver provides a CSI interface that allows Kubernetes clusters running on {aws} to manage the lifecycle of Amazon EFS and Amazon S3 Files file systems.
--

link:efs/latest/ug/whatisefs.html[Amazon Elastic File System,type="documentation"] (Amazon EFS) provides serverless, fully elastic file storage so that you can share file data without provisioning or managing storage capacity and performance. The https://github.com/kubernetes-sigs/aws-efs-csi-driver[Amazon EFS Container Storage Interface (CSI) driver] provides a CSI interface that allows Kubernetes clusters running on {aws} to manage the lifecycle of Amazon EFS and Amazon S3 Files file systems. This topic shows you how to deploy the Amazon EFS CSI driver to your Amazon EKS cluster.

[#efs-csi-considerations]
== Considerations

* The Amazon EFS CSI driver isn't compatible with Windows-based container images.
* You can't use https://github.com/kubernetes-sigs/aws-efs-csi-driver/blob/master/examples/kubernetes/efs/dynamic_provisioning/README.md[dynamic provisioning] for persistent volumes with Fargate nodes, but you can use https://github.com/kubernetes-sigs/aws-efs-csi-driver/blob/master/examples/kubernetes/efs/static_provisioning/README.md[static provisioning].
* https://github.com/kubernetes-sigs/aws-efs-csi-driver/blob/master/examples/kubernetes/efs/dynamic_provisioning/README.md[Dynamic provisioning] requires https://github.com/kubernetes-sigs/aws-efs-csi-driver/blob/master/CHANGELOG-1.x.md#v12[1.2] or later of the driver. You can use https://github.com/kubernetes-sigs/aws-efs-csi-driver/blob/master/examples/kubernetes/efs/static_provisioning/README.md[static provisioning] for persistent volumes using version `1.1` of the driver on any supported Amazon EKS cluster version (see link:eks/latest/userguide/kubernetes-versions.html[Amazon EKS supported versions,type="documentation"]).
...
* The Amazon EFS CSI driver started supporting Amazon S3 Files file systems from version https://github.com/kubernetes-sigs/aws-efs-csi-driver/blob/master/CHANGELOG-3.x.md#v300[3.0.0].
...
* *For Amazon EFS file systems only*: Attach the `{arn-aws}iam::aws:policy/service-role/AmazonEFSCSIDriverPolicy` managed policy.
* *For Amazon S3 Files file systems only*: Attach the `{arn-aws}iam::aws:policy/service-role/AmazonS3FilesCSIDriverPolicy` managed policy.
* *For both Amazon EFS and Amazon S3 Files file systems*: Attach both the `AmazonEFSCSIDriverPolicy` and `AmazonS3FilesCSIDriverPolicy` managed policies.
...
The following example attaches the `AmazonEFSCSIDriverPolicy` for Amazon EFS file systems. If you're using Amazon S3 Files file systems, replace the policy ARN with `{arn-aws}iam::aws:policy/service-role/AmazonS3FilesCSIDriverPolicy`. If you're using both file system types, run the command twice with each policy ARN.
...
The following example attaches the `AmazonEFSCSIDriverPolicy` for Amazon EFS file systems. If you're using Amazon S3 Files file systems, replace the policy ARN with `{arn-aws}iam::aws:policy/service-role/AmazonS3FilesCSIDriverPolicy`. If you're using both file system types, add an additional `--attach-policy-arn` flag with the second policy ARN.
...
.. In the *Filter policies* box, enter `AmazonEFSCSIDriverPolicy` for Amazon EFS file systems, or `AmazonS3FilesCSIDriverPolicy` for Amazon S3 Files file systems.
.. Select the check box to the left of the policy returned in the search. If you're using both file system types, search for and select both policies.
.. Choose *Next*.
...
----
. Attach the required {aws} managed policy to the role with the following command. If you're using Amazon S3 Files file systems, replace the policy ARN with `{arn-aws}iam::aws:policy/service-role/AmazonS3FilesCSIDriverPolicy`. If you're using both file system types, run the command twice with each policy ARN.
+
[source,bash,subs="verbatim,attributes"]
----
...
[source,json,subs="verbatim,attributes"]
----
include::iam_snippet:e34d5873-4f45-4b88-941f-aa6d605a5e16[]
----
.. Create the role.
+
[source,bash,subs="verbatim,attributes"]
...
== Step 3: Create an Amazon EFS file system or Amazon S3 Files file system
...
* To create an Amazon S3 Files file system, see https://github.com/kubernetes-sigs/aws-efs-csi-driver/blob/master/docs/s3files-create-filesystem.md[Create an Amazon S3 Files file system for Amazon EKS] on GitHub.
