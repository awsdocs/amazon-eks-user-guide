...
[TIP]
====
You can also use `kubectl` directly from the EKS console via {aws} CloudShell. Choose *Connect* on any cluster details page to get a pre-configured session. For more information, see <<create-kubeconfig>>.
====
...
For further information, see link:AmazonCloudFront/latest/DeveloperGuide/GettingStartedSimpleDistribution.html[Get started with a basic CloudFront distribution,type="documentation"].
...
----
.. Make sure that the generated checksum in the output matches the checksum in the downloaded `kubectl.sha256` file.
...
----
.. Make sure that the generated checksum in the output matches the checksum in the downloaded `kubectl.exe.sha256` file. The PowerShell output should be an uppercase equivalent string of characters.
