...
----
include::iam_snippet:a8ac9ca8-ab01-49a8-9de7-ab3a2a9195b5[]
----
.. Create the role.
+
[source,bash,subs="verbatim,attributes"]
...
. On the cluster details page for the cluster you created in <<eks-create-cluster,Step 1>>, choose *Connect*.
. {aws} CloudShell opens with `kubectl` pre-configured for your cluster.
...
For more information, see <<create-kubeconfig>>.
...
[source,json,subs="verbatim,attributes"]
----
include::iam_snippet:563b8603-744b-4278-b4ec-a1f971ffd1f0[]
----
...
+
.. For *Name*, enter a unique name for your managed node group, such as [.replaceable]`my-nodegroup`. The node group name can't be longer than 63 characters. It must start with a letter or digit, but can also include hyphens and underscores for the remaining characters.
