...
This add-on requires that `cert-manager` community add-on is deployed on the cluster as a prerequisite. For more information about this add-on, see <<addon-cert-manager>>. For more information about installing add-ons, see <<creating-an-add-on>>.
