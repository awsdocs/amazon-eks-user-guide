...
.. If all of the add-ons that you selected have *Requires subscription* under  *Status*, choose  *Next*. You can't  <<updating-an-add-on,configure those add-ons>> further until you've subscribed to them after your cluster is created. For the add-ons that don't have *Requires subscription* under  *Status*, do the following:
+
...
. Determine the Kubernetes version of your cluster, such as `1.29`.
...
----
+
This output shows that addon version `v1.5.0-eksbuild.1` is compatible with Kubernetes cluster version `{k8s-n}`.
