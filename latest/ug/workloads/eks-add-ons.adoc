...
You can set a custom namespace when creating an add-on. For more information, see <<creating-an-add-on>>.
...
** Community add-ons are not built by {aws}. {aws} only validates community add-ons for version compatibility. 
