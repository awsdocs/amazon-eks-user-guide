...
+
If the *{aws} Marketplace add-ons* that you want to install aren't listed, you can choose the page numbering to view additional page results or search in the search box. In the *Filtering options*, you can also filter by *category*, *vendor*, or *pricing model* and then choose the add-ons from the search results. Once you've selected the add-ons that you want to install, choose *Next*.
...
... If you want to install the add-on into a specific namespace, enter it in the *Namespace* field. For {aws} and community add-ons, you can define a custom Kubernetes namespace to install the add-on into. For more information, see <<custom-namespace>>.
... Choose *Next*.
...
** If the add-on or your cluster is not set up for EKS Pod Identities, use IRSA. <<iam-roles-for-service-accounts,Confirm IRSA is set up on your cluster.>>
...
** If you want to install the add-on into a custom Kubernetes namespace, add the `--namespace-config 'namespace=<my-namespace>'` option. This option is only available for {aws} and community add-ons. For more information, see <<custom-namespace>>
