...
** *IP*
 – Registers [.noloc]`Pods` as targets for the ALB. Traffic reaching the ALB is directly routed to [.noloc]`Pods` for your service. You must specify the `alb.ingress.kubernetes.io/target-type: ip` annotation to use this traffic mode. The IP target type is required when target [.noloc]`Pods` are running on Fargate.
