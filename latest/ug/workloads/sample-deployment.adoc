...
:info_titleabbrev: Sample deployment (Linux)

In this topic, you deploy a sample application to your cluster on Linux nodes.

== Prerequisites

* An existing Kubernetes cluster with at least one node. If you don't have an existing Amazon EKS cluster, you can deploy one using one of the guides in <<getting-started>>. 
...
Create a Kubernetes deployment. This sample deployment pulls a container image from a public repository and deploys three replicas (individual Pods) of it to your cluster. To learn more, see https://kubernetes.io/docs/concepts/workloads/controllers/deployment/[Deployments] in the Kubernetes documentation.
...
. View the details of one of the Pods listed in the output when you <<sample-app-view-namespace,viewed the namespace>> in a previous step. Replace [.replaceable]`65b7669776-m6qxz` with the value returned for one of your Pods.
+
[source,bash,subs="verbatim,attributes"]
----
