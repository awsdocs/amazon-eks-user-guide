...
[abstract]
--
Set up an Amazon EKS cluster with GPU compute, monitoring, and an Amazon S3 bucket for model storage.
--
...
This section guides you through creating an Amazon EKS cluster ready to run inference workloads, including the compute with GPUs, monitoring stack, and Amazon S3 storage for model weights, along with the necessary permissions.
...
For step-by-step instructions using the {aws} CLI, see <<ml-cluster-setup-cli,Cluster Setup via CLI>>.
