...
[abstract]
--
Learn how to manage specialized hardware devices such as NVIDIA GPUs, {aws} Trainium chips, and Elastic Fabric Adapter (EFA) devices in Amazon EKS clusters using Dynamic Resource Allocation (DRA) and Kubernetes device plugins.
--
...
NVIDIA DRA driver:: The link:https://github.com/kubernetes-sigs/dra-driver-nvidia-gpu[NVIDIA DRA driver for GPUs] on GitHub enables flexible allocation and dynamic configuration of NVIDIA GPUs. See <<device-management-nvidia-dra-device-plugin>> for information on managing GPUs with the NVIDIA DRA driver and <<ml-eks-nvidia-ultraserver>> for information on using `ComputeDomains` for Multi-Node NVLink (MNNVL) workloads with EC2 Grace-Blackwell instances.

EFA DRA driver:: The EFA DRA driver (link:https://github.com/kubernetes-sigs/dranet[DRANET] on GitHub) manages Elastic Fabric Adapter (EFA) device allocation with topology-aware scheduling that pairs EFA interfaces with their topologically-local GPUs or Neuron devices, and supports device sharing between Pods. For more information, see <<device-management-efa>>.

Neuron DRA driver:: The Neuron DRA driver manages {aws} Trainium and {aws} Inferentia2 device allocation with topology-aware scheduling, connected device subset allocation, and Logical NeuronCore (LNC) configuration, without requiring custom scheduler extensions. For more information, see <<device-management-neuron>>.
...
NVIDIA device plugin:: The link:https://github.com/NVIDIA/k8s-device-plugin[NVIDIA device plugin] on GitHub advertises NVIDIA GPUs as `nvidia.com/gpu` extended resources and tracks the health of GPUs.
...
* DRA is available on Amazon EKS with Kubernetes version 1.33 and above, but it is recommended for Kubernetes versions 1.34 and later because of an upstream link:https://github.com/kubernetes/kubernetes/issues/133920[Kubernetes issue] on GitHub. Your cluster control plane and nodes must be running a Kubernetes version that supports DRA.
