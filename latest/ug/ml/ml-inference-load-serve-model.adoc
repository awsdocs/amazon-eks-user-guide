...
In this section, you deploy a large language model (LLM) to Amazon EKS, serve it, and interact with the inference endpoint. You use the cluster you created in the <<ml-cluster-setup,Cluster setup>> section. The instructions in this section work for both EKS Auto Mode and self-managed Karpenter.
...
This deployment uses the following {aws} DLC for https://gallery.ecr.aws/deep-learning-containers/vllm[vLLM 0.21.0] with SOCI support:
