...
You watch the load test in the pre-loaded vLLM dashboard, so make sure the Grafana load balancer you set up in the <<cluster-setup-cli-grafana-loadbalancer,Access Grafana>> section is reachable. Print its URL:
