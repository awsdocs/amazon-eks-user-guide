...
*Access Grafana.* You watch the load test in the pre-loaded vLLM dashboard, so make sure the Grafana load balancer you set up in the <<cluster-setup-cli-grafana-loadbalancer,Access Grafana>> section is reachable. Retrieve its hostname:
...
Open Grafana (open the load balancer hostname; see <<cluster-setup-cli-grafana-loadbalancer,Access Grafana>> for details) and navigate to *Dashboards > GPU Monitoring > Performance Testing - vLLM Load Analysis*. The *vLLM Request Rate* panel shows the burst of warm-up traffic. The *vLLM Average Latency* panel shows that the first request is slower, which is the one-time GPU warm-up cost.
...
Open Grafana (open the load balancer hostname and log in as `admin`; see <<cluster-setup-cli-grafana-loadbalancer,Access Grafana>> for details) and navigate to *Dashboards > GPU Monitoring > Performance Testing - vLLM Load Analysis*. This dashboard reads from Amazon Managed Service for Prometheus, so panels can lag the live state by up to a minute because metrics are remote-written in batches.
