...
The kube-prometheus-stack from the <<cluster-setup-cli-monitoring,Monitoring>> setup runs in the `monitoring` namespace.
...
The `vllm-inference-app` Deployment and `vllm-inference-svc` Service from <<ml-inference-load-serve-model,Load & Serve Models>> run in the `default` namespace.
...
You watch the load test in the pre-loaded vLLM dashboard, so make sure the Grafana load balancer you set up in the <<cluster-setup-cli-grafana-loadbalancer,Access Grafana>> section is reachable. Retrieve its hostname:
...
The `vllm-inference-app` ServiceMonitor, created in the <<ml-inference-load-serve-model-monitoring,Monitor vLLM>> step, runs in the `default` namespace and tells Prometheus where to scrape vLLM metrics.
