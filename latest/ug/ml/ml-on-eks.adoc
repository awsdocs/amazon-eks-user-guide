...
To begin planning for and using AI/ML platforms and workloads on Amazon EKS, follow the <<ml-cluster-setup,Cluster setup>> section to create an Amazon EKS cluster, including the required Kubernetes components, in your {aws} account. Once your environment is up and running, you can continue to the next steps:

* <<ml-cluster-setup,Cluster setup>>: Create the Amazon EKS cluster, monitoring, and Amazon S3 bucket infrastructure to use throughout this section.
* <<ml-inference,Inference>>: Use Amazon EKS to deploy, configure, and start using an inference application with a large language model (LLM).
* <<ml-cluster-configuration,Cluster configuration>>: Configure Amazon EKS clusters optimized for AI/ML workloads.
* <<ml-compute-management,Capacity management>>: Manage and optimize compute resources for machine learning workloads on Amazon EKS.
* <<device-management,Device management>>: Manage specialized hardware devices using Dynamic Resource Allocation (DRA) and device plugins.
