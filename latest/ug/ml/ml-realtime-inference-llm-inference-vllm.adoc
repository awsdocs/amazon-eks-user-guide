...
= High-throughput LLM inference with vLLM on Amazon EKS
:info_titleabbrev: LLM inference with vLLM

[abstract]
--
This quickstart guide provides a walkthrough for deploying Large Language Models on Amazon EKS using vLLM and GPUs for text-based real-time inference applications.
--
...
Create a random name for a new S3 bucket that will be created. Once created, use this same bucket name for all steps in this guide. Example:
...
Create IAM policy to allow S3 mount point to read from your S3 bucket:
...
Find the IAM role name used by S3 Mountpoint CSI by checking S3 CSI Driver service account annotations:
...
Attach your IAM policy with S3 Mountpoint CSI role:
...
Create Persistent Volume (PV) and Persistent Volume Claim (PVC) to provide read-only access to the S3 bucket across multiple inference pods. The ReadOnlyMany access mode ensures concurrent access to model files, while the CSI driver handles the S3 bucket mounting:
...
AWS G5 instances with A10G GPUs and G6 instances with L4 GPUs both provide 24GB VRAM per GPU, sufficient for loading Llama 3.1 8B Instruct weights. If you are deploying a model with larger weights, consider using a multi-GPU instance type or a multi-node setup.
...
Review pod logs to check GPU details are listed, similar to output below (not necessarily the same GPU model): 
...
Check that vLLM container is in `Ready 1/1` state:
...
Expose the inference endpoint locally through Kubernetes port forwarding for local development and testing. Leave this command running in a separate shell:
...
Need to install AWS Load Balancer Controller? Follow the deployment steps in the https://docs.aws.amazon.com/eks/latest/userguide/aws-load-balancer-controller.html[AWS EKS documentation].
...
Validate the inference container functionality locally through the forwarded port.
Send a connection request and ensure that the response includes HTTP code 200:
...
For demonstration purposes, this guide runs a sample chatbot application using project https://github.com/yoziru/nextjs-vllm-ui[nextjs-vllm-ui] to showcase user interactions with the model. 
...
Apply these changes to the running deployment using a kubectl patch command.
...
Wait for vLLM pods to transition to Ready state:
...
Then run guidellm using the same benchmarking values as before:
...
The optimized vLLM configuration significantly improved inference throughput (RPS) and reduced latency (E2E, TTFT), with only a minor millisecond increase in tail latency (TPOT). These results demonstrate how vLLM significantly improves inference performance, allowing each container to process more requests in less time for cost-effective operation.
