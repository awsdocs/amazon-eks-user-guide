...
== {aws} managed policy: AmazonEKSMCPReadOnlyAccessPolicy
:info_titleabbrev: AmazonEKSMCPReadOnlyAccessPolicy

Attach `AmazonEKSMCPReadOnlyAccessPolicy` to your IAM entity to enable read-only tools in the Amazon EKS MCP Server. Before connecting to the service, configure local {aws} credentials for an IAM role with this policy attached. This policy grants the permissions needed to call downstream APIs for read-only tools.
...
To view the latest version of the JSON policy document, see link:aws-managed-policy/latest/reference/AmazonEKSMCPReadOnlyAccessPolicy.html#AmazonEKSMCPReadOnlyAccessPolicy-json[AmazonEKSMCPReadOnlyAccessPolicy,type="documentation"] in the {aws} Managed Policy Reference Guide.
...
|Introduced new `AmazonEKSMCPReadOnlyAccessPolicy` policy.
|November 20, 2025
