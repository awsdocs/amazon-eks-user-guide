...
[#security-iam-awsmanpol-amazoneksmcpfullaccess]
== {aws} managed policy: AmazonEKSMCPFullAccess
:info_titleabbrev: AmazonEKSMCPFullAccess

Attach `AmazonEKSMCPFullAccess` to your IAM entity to enable all tools in the Amazon EKS MCP Server. Before connecting to the service, configure local {aws} credentials for an IAM role with this policy attached. This policy grants the permissions needed to call downstream APIs for all tools, including privileged tools that may involve write operations (e.g., `manage_eks_stacks`, which creates an EKS cluster and associated EC2 and IAM resources) and read-only tools.

*Permission details* 

This policy grants permissions to use all tools in the EKS MCP Server, including both privileged tools that may involve write operations, and read-only tools in <<eks-mcp-tool-configurations>>.
...
Attach `AmazonEKSMCPReadOnlyAccess` to your IAM entity to enable read-only tools in the Amazon EKS MCP Server. Before connecting to the service, configure local {aws} credentials for an IAM role with this policy attached. This policy grants the permissions needed to call downstream APIs for read-only tools.

*Permission details*

This policy grants permissions to use *only* read-only tools in the EKS MCP service meant for observability, troubleshooting, retrieving EKS resource information, and getting EKS-optimized suggestions for the tools in <<eks-mcp-tool-configurations>>.
