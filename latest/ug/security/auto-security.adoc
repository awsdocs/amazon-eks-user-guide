...
* When using tag selectors for subnets and security groups in EKS Auto Mode, limit the IAM permissions of the cluster role to only allow `ec2:CreateNetworkInterface` with specific subnets and security groups.
