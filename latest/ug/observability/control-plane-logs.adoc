...
[NOTE]
====
Amazon EKS control plane logs are delivered to CloudWatch Logs in a few minutes. However, log delivery is on a best effort basis, and your logs might be delayed beyond the typical delivery time.
====
