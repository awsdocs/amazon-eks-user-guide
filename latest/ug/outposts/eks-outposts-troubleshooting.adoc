...
Amazon EKS automatically updates all existing local clusters to the latest platform versions for their corresponding [.noloc]`Kubernetes` minor version. For more information about platform versions, please refer to link:eks/latest/userguide/eks-outposts-platform-versions.html[Learn Kubernetes and Amazon EKS platform versions for AWS Outposts]. 
...
* One of the new [.noloc]`Kubernetes` control-plane instances may have experienced an unexpected bootstrapping failure. Please file a ticket with link:https://console.aws.amazon.com/support/home[{aws} Support Center] for further guidance on troubleshooting and log-collection in this exceptional case. 
...
📝 https://github.com/search?q=repo%3Aawsdocs%2Famazon-eks-user-guide+%5B%23eks-outposts-troubleshooting%5D&type=code[Edit this page on GitHub]