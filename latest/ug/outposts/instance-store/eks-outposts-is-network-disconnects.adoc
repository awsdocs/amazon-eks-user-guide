...
[.topic]
[#eks-outposts-is-network-disconnects]
...
[#eks-outposts-is-network-disconnects-authentication]
...
|No. See <<eks-outposts-is-network-disconnects-irsa>>.

|EKS Pod Identity
|No. See <<eks-outposts-is-network-disconnects-irsa>>.
|===

[#eks-outposts-is-network-disconnects-x509]
=== x.509 client certificates
...
[#eks-outposts-is-network-disconnects-cluster-dns]
...
[#eks-outposts-is-network-disconnects-pod-dns]
...
[#eks-outposts-is-network-disconnects-irsa]
...
[#eks-outposts-is-network-disconnects-etcd]
== `etcd` behavior during disconnects

During network disconnects, `etcd` snapshots cannot be backed up to Amazon S3. If more than one `etcd` instance becomes unavailable during a disconnect, `etcd` loses quorum and Kubernetes API operations are not available until your Outpost reconnects and `etcd` quorum is restored. Workloads that are already running continue to operate.

[#eks-outposts-is-network-disconnects-cp-logs]
...
[#eks-outposts-is-network-disconnects-local-observability]
== Local observability
...
[#eks-outposts-is-network-disconnects-image-repo]
...
[#eks-outposts-is-network-disconnects-pod-failover]
...
[#eks-outposts-is-network-disconnects-simulate]
