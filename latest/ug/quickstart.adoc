...
Create a `cluster-config.yaml` file and paste the following contents into it. Replace `<region-code>` with a valid [Region](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-regions-availability-zones.html#concepts-available-regions), such as `us-east-1`:
...
For more config options, read the `eksctl` [config schema](https://eksctl.io/usage/schema/).
...
📝 https://github.com/search?q=repo:awsdocs/amazon-eks-user-guide+&#91;&#91;quickstart,&type=code[Edit this page on GitHub]